...
- On 2026-08-31, OTB cross-source deduplication began treating terminal DGT noise as one game when target side, normalized players, result, and date agree: mainlines must share at least 12 exact plies and may then be an exact prefix or disagree only within the final two plies of each recording. Earlier or broader divergences remain separate. A supplied real tournament copy ending `59...d3` versus a broadcast tail of `60.Ra4 Ke5 61.Ra1` is pinned as a regression alongside a different-final-move boundary test. The same bounded rule is present in Outpost. En Croissant passed all 27 focused OTB Rust tests and its headless collector passed `cargo check`; current Outpost `main` passed 71 focused importer-module tests, all 75 standalone collector tests, and all-target `headless-otb` checking. En Croissant was not restarted, while the existing Outpost development watcher automatically rebuilt and restarted its debug process after the source edit. No existing library row was changed, and the phone collector source has not yet been published to the installed home-server runtime.

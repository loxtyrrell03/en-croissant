...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-6` and live pipeline 6. The shared desktop/phone adapter now verifies immediate pin/skewer and defender-removal material gains against every legal defence of the named targets, including captures, interpositions, checking replies, supported recaptures, and promotion counterplay. Defender removal must improve the legal exchange; an already pinned pseudo-defender does not qualify. Proven mechanisms outrank generic piece pressure and duplicate capture labels. Promotion evidence requires the actual promoting move/ply. Timelines retain both-side/repeated motifs but stop at checkmate, stalemate or dead material, avoiding arbitrary engine shuffles after a draw. Live fork arrows show both targets, pin/skewer arrows show their ray, and pin labels anchor to the pinned piece; earlier preparatory moves remain visible for later-ply motifs. These relationships use legal geometry, not prose parsing. The earlier architecture remains: one primary lesson, separate conditional ply evidence, losing-alternative filtering, immediate played/best-move comparisons, and bounded quiet mate proofs (4,096 nodes for mate next turn, 16,384 for mate within three). Do not transplant the old full PV under changed defence or treat exhausted proof as absence of tactics. Versioned idle migration and the rebuilt shared-review worker carry the shared adapter. Verification passed 125 focused adapter/review/persistence/phone/render tests, 20 fresh Stockfish 18 depth-16 judgement scenarios (nine original positions, four before/after, two quiet preparations, five material candidates), a restricted faster-mate check, targeted lint, the shared-review worker build, and the 8,851-module production Vite build. Material candidates have both unrestricted and move-restricted engine searches. Whole-project type checking still reaches the unrelated `otbGameImport.test.ts` number/bigint fixture error. `benchmarks/tactical-relevance/README.md` and four JSON reports document evidence and rejected premises; small judgement sets are not general accuracy estimates. The bishop skewer wins a queen but reaches a dead draw, and a prior unrestricted engine search omitted a faster quiet mate confirmed by a restricted search. The broader tuning goal remains active: longer non-mating combinations, defensive resources, candidate coverage, deeper best-move comparisons, and running-app proof remain incomplete. The vendored website v55 primitive classifier is unchanged. No runtime or hosting deployment is asserted.

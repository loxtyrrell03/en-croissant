...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-4` and live pipeline 4. The shared desktop/phone adapter legally replays lines, rejects poisoned captures and capturable geometric forks, and limits main themes to the immediate forcing sequence. Live Tactics excludes losing alternatives and alternatives missing a proven mate, with one main headline/board label. Separate expandable timelines now assess both sides at each legal ply and retain repeated motifs without promoting conditional counterplay into the primary lesson; routine recaptures, generic opening/check tags, and incidental pins are suppressed. Mistake Review compares immediate refutations after the played and best moves using legality, exchange, fork-target, and mate-escape witnesses; a threat that also persists after the best move is not alone blamed on the mistake. Deeper counterfactuals remain unproven, and the old refutation must never be replayed wholesale under changed defence. Saved metadata and the rebuilt shared-review worker retain actor, comparison evidence, and separate timelines; existing idle migration uses the new version. Verification passed 101 focused adapter/review/persistence/phone/render tests, nine fresh Stockfish 18 depth-16/MultiPV3 position judgements plus three before/after judgements, targeted lint, the shared-review worker build, and the 8,851-module production Vite build. Whole-project type checking still reaches the unrelated `otbGameImport.test.ts` number/bigint fixture error. `benchmarks/tactical-relevance/README.md` and both JSON reports record the evidence: O-O removes the screenshot f7 fork, Nxe4 wins a loose queen while saving the knight, and g5 prevents immediate back-rank mate with Kg7. Deterministic tests retain opponent counterforks, repeated forks, and capturable-knight negatives. These small judgement sets are not general accuracy estimates. The broader tuning goal remains active: quiet preparations, pure mate threats, defensive combinations, pawn breakthroughs, and deeper best-move comparisons need stronger proof; unproven zugzwang is withheld. The vendored website v55 primitive classifier is unchanged. No running-app visual proof or hosting deployment is asserted.

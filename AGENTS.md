...
- On 2026-08-30, cold BritBase hydration stopped imposing a fixed 350 ms delay before every immutable archive request. A Michael Binks birth-year import exposed the artificial gate by reaching only 166/314 pending files in 60 seconds even though valid host responses took roughly 50-65 ms; 163 successful cache writes had a 351.9 ms median interval. BritBase now uses explicit 16-file batches with the correct decade Referer, tries every canonical/legacy alias, writes and indexes every successful body, retains deterministic exact-game filtering and fail-closed coverage, and keeps the global gate solely for an upstream `429 Retry-After`. Do not add permanent 404 suppression: three currently advertised ZIPs are absent from both aliases but may be restored. A live 35-file 1940 sample fetched 35/35 and 1,423,272 bytes in 233 ms without 403/429, versus an 11.9-second old-gate floor. The first fixed all-source Binks run completed in 5.368 seconds while hydrating the remaining 148 valid historical archives; the next complete run took 0.707 seconds, with BritBase at 0.248 seconds, 357 exact games and matching `Event`/`OutpostSource`/`OutpostSourceUrl` counts, ten clean source reports, and no coverage gaps. The desktop standalone collector suite passes 68 tests, including a barrier regression proving all 16 bodies are downloaded and cached byte-for-byte before any mock response can complete.
- On 2026-08-30, cold interactive BritBase scans stopped building the all-player SQLite corpus for every raw cached or newly downloaded archive. Indexed files still use one bulk SQLite query; unindexed bytes are filtered in bounded parallel through the same exact identity, year, and PGN pipeline and remain cached. On the shared Wadsworth corpus, the isolated raw-cache path fell from 20.448 seconds to 0.367 seconds, and a fully cold network run took 3.525 seconds; all three outputs remained byte-identical at 338 games and 436,773 bytes (`C4C3D4AC195D664BE2D6C44503D4CC768DB4CCEC7D1F879FCCF36776F5CA842F`). Preserve this indexed/unindexed split so corpus maintenance never returns to the Import button path.
- On 2026-08-30, desktop Lichess FIDE discovery began following the canonical same-player `rel=next` cursor to exhaustion and resolving each advertised last-round card to its complete tournament span. Unique tournaments enumerate every exact player chapter via the player roster endpoint; legacy untagged events try the exact roster name first, each chapter retains a whole-round fallback, and roster failure falls back to the complete tour PGN under strict identity/year filtering. A tournament is skipped only when its full month span is indexed, and ambiguity or no-match results remain coverage failures rather than partial success.
- Live FIDE tournament work is pipelined in deterministic sorted windows of eight: metadata can resolve concurrently, but duplicate-tour choice, player lookup, exact chapter download, and merging are stable across response order. Only an explicitly finished round permits roster-cache reuse, so ongoing events refresh live and cannot silently omit a new round. Lichess requests remain serialized through full-body reads with a 120 ms request-start floor plus `Retry-After` handling to avoid upstream throttling. Preserve desktop's intentional check/mate SAN suffix semantics when synchronizing this collector. The 68 focused collector tests and the all-target headless compile check pass with this behavior.
- On 2026-08-30, desktop live-round discovery began preferring the public broadcast page's embedded tournament, exhaustive round list, and chapter roster over a redundant unauthenticated JSON metadata request, while retaining the JSON endpoint as a compatibility fallback. A finished tournament proven to contain exactly one round can import its exact player chapters directly; ongoing and multi-round tournaments still require the complete fresh player roster. Exact and whole-round study exports explicitly retain comments, clocks, variations, and orientation, and ambiguous chapter matches keep the lossless whole-study fallback. This port preserves desktop's intentional `+` versus `#` SAN fingerprint semantics; 70 focused headless collector tests, the all-target headless compile check, Rust formatting, and diff checks pass. A desktop button-to-ready live benchmark was not part of this source-parity milestone.

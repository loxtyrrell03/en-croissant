...
- On 2026-08-31, desktop Plan Explorer clarified the distinction between its aggregate score and local analysis. `Combined score` is now the default order and column label; the settings surface calls its presets Balanced, Engine-led, and Games-led, labels the adjustable percentage `Engine evidence share`, and disables that percentage outside Balanced mode because the two presets use fixed mixes. The Plan Explorer switch is `Use engine evidence`, explains that it automatically runs the selected local engine, reports starting/analysis progress/PVs ready, and labels the separate column `Engine verdict`. Existing Bayesian game-sample shrinkage, neutral handling for unmatched engine rows, CP-loss safety gate, side/view lifecycle keys, and request-driven engine cleanup remain unchanged. Outpost received the equivalent clarity through its explicit-Run architecture plus responsive score/ranking fixes. The PC-hosted phone companion has no full Plan Explorer surface, so no phone-only control was invented. Verification passed all 21 focused plan-strength tests, targeted formatting and lint with zero findings, TypeScript checking, the OTB-prep build, and the 8,840-module production Vite build. No GUI automation, physical desktop interaction, phone release, or deployment was performed.

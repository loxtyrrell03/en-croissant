...
- On 2026-08-31, En Croissant gained the native read-only half of the shared local Lichess data store at `%LOCALAPPDATA%/ChessData/lichess`. `getLocalLichessOpeningDbStatus` and `queryLocalLichessOpening` consume Outpost's versioned SQLite format for Lichess All, the broadcast-based local Masters corpus, and exact player/color queries; an installed exact-position miss is authoritative and must not silently fall through to the online provider. Availability is source-scoped, so installing only broadcasts does not mask the still-uninstalled Lichess All family. The reader uses matching stable position hashes and replays compact per-player UCI lines through shakmaty. The existing compact cloud-eval builder now writes to the shared `evaluations` folder by default, while lookups retain a legacy per-app manifest fallback. Generated Specta bindings include the new command/types, three native format/coverage tests pass, and the debug native binary compiled and exported bindings through the headless-only export path. This milestone is the native/storage boundary only: desktop/Plan/Prep and PC-hosted phone routing are completed in the following frontend milestone, and no database download, runtime deployment, or physical UI proof is implied here.

- On 2026-08-31, the shared local Lichess opening snapshot became the first source for every En Croissant Explorer consumer: Database and Compare, Plan Explorer, Prep, Engine Plans practical evidence, Opening Health, AI Coach opening context, and the PC-hosted phone Database/Prep surfaces. A source-covered local response is authoritative even when the exact position is empty; only `available: false` permits the existing paced online fallback, while an unreadable installed snapshot fails closed. Desktop fallbacks require the least-privileged linked Lichess token, and source-specific UI gates no longer block a covered local All, Masters, or player query. Masters labels explicitly identify the local corpus as a broadcast-based snapshot. Outpost owns the single atomic download/index lifecycle for the shared store, so En Croissant's missing-source guidance points to Outpost Settings > Sources instead of starting a competing multi-gigabyte build. The home server exposes private-origin `GET /api/lichess/opening/status` and `POST /api/lichess/opening` routes backed by a read-only `node:sqlite` reader of the same shared database; phone requests use them before either the PC Explorer proxy or direct fallback, including hidden child prefetches. Verification passed five desktop local-first contract tests, 14 phone offload tests, four Node store tests, three native format/coverage tests, 174 related plan/prep/engine/opening-health tests, whole-project TypeScript checking, Node syntax/CORS/lane checks, and the 8,841-module production build. One unrelated existing phone companion assertion still expects the old `Engine best` wording instead of current `Local eval best`. No snapshot download/install, service deployment, or physical UI proof is implied by this integration milestone.

- On 2026-08-31, En Croissant's desktop and PC-hosted phone Explorer fallbacks adopted the same provider-safe Lichess transport policy as Outpost. Desktop Lichess All, Masters, player, Plan Explorer, and Prep requests share one priority-aware lane with one active request, at least 500 ms between starts (1,000 ms for player indexing), abortable stale work, and a provider-wide cooldown of at least 60 seconds after HTTP 429. Prep waits briefly for navigation to settle and limits only automatic remote After-prep enrichment to ten projection and six supporting lookups per viewed position; explicit tools remain complete, local database work is unbounded, and visible root requests overtake queued background mining. The phone now keeps a bounded result cache, prefetches only the single most likely child, serializes PC-server upstream requests through the same pacing/cooldown policy, returns Retry-After to clients, and never bypasses a PC 429 with an immediate direct retry. The shared local Lichess database owns the default/source implementation; these paths remain the online fallback and must not regain a token prerequisite when a local result is available. Verification passed 79 focused desktop/phone tests, all 81 PC-server Node tests, Node syntax checking, and the 8,840-module production Vite build. The full web suite retained 686 passes and its three unrelated baseline failures in engine-detail wording, audio mocking, and timestamp expectations; whole-project TypeScript checking reaches only the existing unrelated `number` versus `bigint` fixture error in `otbGameImport.test.ts`. Outpost parity is pushed as `4918ec1`.

- On 2026-08-31, En Croissant desktop gained a lazy-loaded `Tactics` board-analysis tab backed by the same vendored/versioned v54 tactical classifier used by Mistake Review. Opening the tab runs one bounded depth-16 scan of the current position with the selected saved local engine, preserves profile-specific UCI options such as LC0 `WeightsFile`/`MinibatchSize`/`NNCacheSize` while forcing only `MultiPV=1`, classifies the returned principal variation, caches the completed position, and terminates its dedicated engine process after completion, navigation, engine changes, or unmount. Scan completion is authoritative from the request-matched terminal `bestMovesPayload` event because a new native `getBestMoves` invocation remains open for the lifetime of its engine reader; the listener is installed before search starts so fast results cannot be missed. A six-second wall-clock deadline accepts the latest principal variation only once it has reached depth 8, otherwise it shows an explicit failure and terminates the engine, so the tab cannot remain on the animated 99% state indefinitely. Specific motifs such as forks, pins, interference, deflection, removal of the defender, sacrifices, promotion tactics, zugzwang, and named mating patterns are shown as confidence-labelled cards; generic line tags such as check, capture, length, and evaluation are suppressed so ordinary moves do not become false tactical alerts. When a motif is found, the board displays a labelled theme pill at the triggering square plus distinct attacker, reply, and trigger arrows through the relevant forcing plies. Focused classifier/Mistake Review tests pass 54/54, including terminal-event completion, deadline fallback, real named-mate, and rook-fork board-cue regressions; touched-file formatting/lint passes, and the 8,840-module production Vite build passes. Whole-project TypeScript checking reaches only the existing unrelated `number` versus `bigint` fixture error in `otbGameImport.test.ts`. This is a desktop-only board-analysis surface: the PC-hosted phone companion has no equivalent full analysis-tab workspace and Outpost is a separate product, so no unsupported parallel UI was invented. Source/build verification is complete; no post-fix GUI automation or running-app visual proof was performed.

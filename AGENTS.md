...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-19` and live pipeline 19. Real CSh8J now prioritizes Nd5+'s promotion-backed Fork: king replies lose Nb6, whereas Nxd5 abandons c8 and permits promotion. A nominated seventh-rank pawn must have no profitable immediate promotion (all four roles checked before the fork) and a promotion square guarded by a fork target. The existing bounded legal exchange proof checks every reply against the named fork targets or that same pawn; unknown/check-tempo-only results abstain. A concrete defender-captures-forker branch must allow sound queen promotion. This does not certify arbitrary endings or underpromotion-only escapes. The verified minimum is 320 cp, distinct from PV endpoint material or whole-position engine evaluation. That proof supplies headline evidence/value, causal material comparisons and board arrows for both fork targets, the defender's promotion-square link and the pawn push; promotion stays secondary at its actual ply. Eight new regressions include both colours, truncated/alternative lines, a missing pawn, a second promotion guard and an unrelated passer. Fresh before/after Stockfish checks identify the missed primary after Kxg3 and allowed primary after Kf6, with Nc8 preventing the immediate fork. Only CSh8J's headline changes in the frozen 32-position expansion; G8wdr's queen-exchange fork, w1lKu's quiet pin and other recorded misses remain open. Verification passed 224 selected regression tests in 23 files, all ten opt-in engine tests, targeted formatting/lint, shared-review worker and 8,851-module production builds. Type checking retains only the known unrelated OTB number/bigint fixture error. The shared desktop/phone worker is rebuilt; website primitives, Outpost and live hosting/runtime are unchanged. No running-app/GUI proof or general accuracy claim is made. Details and refreshed diagnostics remain under `benchmarks/tactical-relevance/`.

...
- On 2026-08-30, Lichess FIDE listing pagination stopped synthesizing `/fide/{id}/player?page=N`. Lichess redirects that placeholder slug to the canonical player URL while dropping the query, so the old loop fetched page 1 twice and could silently omit every later page; the earlier 7.862-second Kasparov live-lane result therefore was not a complete quality baseline. Discovery now follows the exact same-origin, same-FIDE-player `rel=next` cursor advertised by each response, decodes HTML entities, tracks visited cursors, and reports loops, conflicting/invalid cursors, or the 20-page cap as coverage errors. Keep this cursor validation and the exact metadata-to-chapter PGN path: a direct whole-round shortcut was rejected because it previously regressed large events from 3.105 seconds to 69.818 seconds. The standalone collector suite passes 61 tests after the pagination regressions were added.
- On 2026-08-30, live Lichess FIDE work became a bounded metadata-to-exact-PGN pipeline instead of resolving a player's entire listing before downloading the first game. At most eight round tasks are active, the shared network lane remains strictly one request through the complete response body, exact study chapters retain whole-round fallback, and completed tasks merge in deterministic URL order. Successful responses no longer reserve an unconditional 120 ms next-start slot; explicit `Retry-After` and server-error cooldowns remain fail-closed. The standalone collector suite passes 64 tests, including full-body serialization, cooldown, bounded ordering, deterministic merge, and annotated fallback regressions.

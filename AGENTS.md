...
- On 2026-09-12, adapter 82 / live pipeline 87 adds king-safe interference: positively test the original guard's role in an illegal king capture, then cover every legal defence under 4,096 visits and two further attacking replies. Both guard/blocker exchange directions may recover targets originally defended by that same guard; quiet retention only saves material already captured. The real Rg5 rook ending now leads with Interference, its pawn payoff stays at the actual matching ply, and rook exchanges are not free-piece badges. Existing forced self-interference and secondary mating deflection remain. Forty fresh engine searches include a rejected assumption that a pawn-winning bishop ending must be winning overall; local material bounds are not game evaluations. Other 19 rare results, all 180 private exact-input source/live results, 32 priorities and 135 prior public worker headlines are unchanged, not certified accuracy successes. Verification: 1,352 selected tests, 509 production-worker inputs, lint, builds, three service and two dev-cache tests pass. Public worker median/p95/max is 80/214/546 ms excluding engine/UI; cold HTTP first/next startup is 8,478/69 ms. See benchmarks/tactical-relevance/rare-theme-review.md, private-course-review.md and built-worker-adapter82.json for evidence and limits. Genuine zugzwang, quiet/rare combinations, the compensating-exchange false positive, the unrelated OTB type error and native verification remain open. No app/package/service was restarted or deployed; paid data stays private.

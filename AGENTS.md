...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-26` and live pipeline 26 after an output-blind ordinary-game audit. All three standard games from one complete public archive are frozen as legal UCI mainlines; every fifth reached ply from 8 through 60 gives 24 alternating-side samples without score/result/theme selection. Fresh before/after searches and actual previous-move context exposed an incidental skewer admitted merely because the PV ended in mate, and a compensated bishop/knight recapture mislabelled as hanging a piece. Skewers now require independent material evidence; mating pins require a concrete forbidden-capture counterfactual or material evidence. Root classification and its timeline share the existing compensated-capture rule only after previous-move replay matches the exact current FEN, preserving missing-history caution, unrelated genuine captures and mating recaptures. Live mate-in-one positions omit longer mating alternatives. The new sample has 17 empty live headlines, six accepted immediate lessons, and one explicitly unresolved primary: f7+'s quiet/promotion mate in seven is still reduced to a smaller checking-fork preparation. Long-mate priority and promotion-role necessity must not be called solved. The earlier frozen mixed 32-puzzle headlines are unchanged. Twenty-nine new sample/negative/positive regressions, 332 selected tests in 30 files, all fifteen fresh-engine tests, targeted format/lint, shared desktop/phone review worker and 8,851-module production builds pass; TypeScript retains only the unrelated OTB number/bigint fixture error. Full judgements and reproducible artifacts are in `benchmarks/tactical-relevance/ordinary-games-review.md` and `README.md`. The fixture is development evidence from one small account sample, not general accuracy; routine verification must not refetch or reselection-tune it. Website primitives, Outpost and live runtimes are unchanged; no deployment or end-to-end UI proof is claimed.

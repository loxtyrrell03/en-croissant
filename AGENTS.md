...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-20` and live pipeline 20. G8wdr's earlier fork judgement was incomplete: Ne5 opens Rd8 against Nd4 while attacking Qd3/Bc4, overloading the queen's shared defence. Discovered Attack is primary; Qf1 loses Nd4, whereas Qc3 permits Qxc3 and the subsequent piece win. The fallback nominates only the uncovered battery/mover targets, checks every reply, and permits a capture of a shared defender followed by the existing defender-removal proof. All nested visits share an 8,192-node budget and up to two checking evasions; lower budgets cannot borrow cached success. Exchange leaves debit the stronger immediate recapture or off-square participant loss, not both simultaneously. After a checking sacrifice donates material, an actual legal non-capture can prove retention when the remaining target escapes; checking evasions track the responding piece. Existing defender-removal callers keep their one-check limit. The verified local minimum is 150 cp, not a full-position evaluation. Board arrows show the root battery and knight attacks, not a speculative queen-exchange destination. The real Nxd4 mistake and missed Ne5 after Be7 have fresh before/after engine proof. A ply-5 conditional fork previously displaced the latter's proved root lesson; immediate lessons now outrank later material motifs in either direction, while mating consequences retain priority. Later-only allowed material explanations explicitly state their dependence on preceding replies. Adapter 19's promotion-backed fork remains: CSh8J checks every reply against named fork targets or a previously unprofitable promotion guarded by a target, with a 320 cp minimum and promotion secondary at its actual ply. Only G8wdr's headline changes in the frozen 32-position expansion. Verification passed 237 selected regression tests in 24 files, all eleven opt-in Stockfish tests, targeted formatting/lint, the shared desktop/phone review worker and 8,851-module production builds; TypeScript retains only the unrelated OTB number/bigint fixture error. Thirteen new regressions cover branch independence, participant removal, budgets, retained gains, arrows and primary ranking. `benchmarks/tactical-relevance/README.md`, `expanded-review.md` and fresh reports document the correction and limits. The quiet pin in w1lKu, longer combinations, named-mate overlap, broader coverage and real runtime proof remain open. Website primitives, Outpost and live hosting/runtime are unchanged; no general accuracy or deployment claim is made.

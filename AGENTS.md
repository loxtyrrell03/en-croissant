...
- On 2026-09-12, the additive save_otb_database native command now builds a private .pending database, cleans duplicates/empty records, checks integrity and publishes without replacing an existing destination. A saved job/content/metadata identity makes completed retries read the current game count without appending or restoring removed games; failures retain the source PGN. Both checkouts passed 12 new native file/converter/export checks, five existing cleanup/partial-parser regressions, TypeScript and frontend builds. Scoped Rust formatting passes; existing compile/build warnings remain. The desktop panel has NOT switched to this command yet: source filename reservation, retained job/save checkpoints and refresh/Prep handoff recovery are still required. No installed runtime, owner data, shared collector algorithm, phone behavior or deployment changed. See docs/DESKTOP_OTB_SAVE.md.

- On 2026-09-12, OTB source duration keeps its native u64 measurement and JSON numeric wire format, with a field-level Specta override so desktop regeneration consistently exports elapsedMs as number. The regression checks serde output above u32 range and exact generated source-report text using the production BigInt export policy; other bigint fields are unchanged. It reproduces the old export mismatch and passes in both checkouts, along with TypeScript, eight import utility tests, scoped Rust formatting and both frontend builds. This resolves the primary elapsedMs type error mentioned in earlier milestones; those notes describe their historical checks. No source collection, desktop/phone runtime, owner data or deployment changed. Desktop conversion and the wider audit remain open.

- On 2026-09-12, phone OTB tracking now recovers from confirmed missing jobs and unreadable/other-PC records. Known missing responses stop polling and expose Retry loading; generic failures remain distinct. Set aside search reviews the consequence, rechecks missing jobs, and atomically keeps the exact tracking details while clearing selection. Undo before another selection, explicit details download, failed-write retry and navigation retention are supported. Stored details remain with a new search; changed selections and delayed FIDE preflights cannot replace newer work. Both checkouts passed 77 focused tests, ten new renderer groups and all 26 existing start/lifecycle/workspace groups at 1100/760/360px and 200% text. Isolated type/build passed; primary retains the unrelated elapsedMs type error. Five scoped files lint cleanly. No PC collector/API format, owner data, runtime restart or deployment changed. Malformed artifacts, desktop conversion, multi-tab whole-workspace persistence and the wider audit remain open. See docs/PHONE_OTB_LIFECYCLE.md.

- On 2026-09-12, phone workspace saves now wait for IndexedDB transaction completion. A tab-scoped writer serializes edits, retains failed drafts and exposes Retry save; failed/invalid loads stay gated with Retry loading instead of overwriting saved games. Automatic OTB completion stores its receipt with the games/Prep, preserves replayed edits and deliberate removals, and reports saved only after commit without moving the active board. Update reloads defer while state is unready/unsaved; page hiding flushes best-effort. Both checkouts passed 59 focused tests and seven actual-WebApp/native-IndexedDB renderer groups at 1100/760/360px and 200% text. Isolated type/build passed; primary retains its unrelated elapsedMs type error. Scoped lint is clean; 25 WebApp warnings match baseline. This is phone browser persistence only, not cross-tab whole-workspace coordination or desktop conversion recovery. Tracking recovery is recorded above; malformed artifacts and broader storage repair remain open. No owner data, runtime restart, deployment or real import changed. See docs/PHONE_OTB_LIFECYCLE.md.

- On 2026-09-12, phone OTB starts now persist the original request and UUID before PUT, and a shared session retains ownership across navigation/reload. Retry uses the same ID; source/year/player fields restore, old replies cannot replace another tab's selection, and failed acknowledgement saves retain a recoverable request. The complete record is authoritative over the legacy ID mirror; WebApp subscribes to that selection. Same-origin Web Locks serialize only storage, with visible unsupported/blocked-storage/older-PC errors and no POST fallback. Status/start/Stop reads have 15-second bounds; large artifact downloads have a separate ten-minute bound. Locked fields use readable theme text and scale at 200%. Both checkouts passed 60 focused tests, ten new renderer groups, nine existing lifecycle groups and eighteen FIDE groups. Isolated type/build passed; primary keeps its unrelated elapsedMs type error. Scoped lint is clean apart from 25 unchanged WebApp baseline warnings. Subsequent workspace and tracking recovery are recorded above. No owner runtime restart, deployment or real import. See docs/PHONE_OTB_LIFECYCLE.md.

- On 2026-09-12, the PC OTB service added recoverable PUT starts at an explicit UUID job ID. Concurrent retries join one launch; reused IDs must match normalized player, FIDE ID, year and sources. Retries preserve running/completed/stopped outcomes and can finish a failed initial/status save without duplicating a collector. Stop during the queued save now prevents a later launch. Existing POST/GET/DELETE/artifact routes remain supported; restart recovery still reports interrupted work instead of restarting it. Both checkouts passed 39 service/HTTP tests and scoped lint. Three baseline cases reproduce duplicate launches, the queued Stop race and the previously unsupported endpoint. The subsequent phone integration uses this endpoint; further lifecycle evidence is recorded above. No owner runtime restart, native import or deployment. See docs/PHONE_OTB_LIFECYCLE.md.

- On 2026-09-12, phone OTB Stop retains the returned job and completed PC artifact; shared watchers reject older GETs after cancellation. Stored jobs load before another search and restore player/ID/year. Status identity/state validation, separate artifact-loading state, retained action errors, synchronous Stop/Analyze ownership and unmount retention passed nine real-component groups in both checkouts at 1100/760/360px and 100/200% scale. Action labels and game names wrap; PC-ready copy no longer guesses Prep success from a receipt. Both 39 focused frontend and 25 service tests pass, including a completed-artifact Stop regression; existing eighteen-group FIDE renderer suites pass. Isolated type/lint/frontend build passed; primary retains its unrelated elapsedMs type error. PC cancellation intentionally abandons active source collection instead of promising partial results. Start-request/storage/automatic Prep recovery and desktop conversion remain follow-ups. No runtime/service restart, deployment or real import. See docs/PHONE_OTB_LIFECYCLE.md.

- On 2026-09-12, the desktop FIDE profile modal adopted the shared bounded lookup path with explicit ambiguous-match selection, editable search/retry, stale-request rejection and Back focus restoration. Board/Game Info use saved WhiteFideId/BlackFideId tags, including blank-name games. Entry controls are keyboard accessible and native profile links expose opening failure/retry with duplicate/late-error protection. Rating cards wrap at 200% text. Both checkouts passed ten real-component profile groups and 26 focused FIDE tests; isolated type/lint/frontend build passed. Primary retains its unrelated elapsedMs number/bigint type error. Native browser launching, owner data and runtimes were untouched. The phone has no separate profile modal; its import picker parity is recorded above. See docs/FIDE_LOOKUP_RECOVERY.md.


- On 2026-09-12, FIDE import lookup failures became retryable in desktop and phone. Both forms share exact-ID/unique-name resolution and reject missing/conflicting IDs before starting work; optional name-only imports and full-career/manual-year behavior remain supported. Autofill and preflight cancel on replacement/unmount, and Stop FIDE search prevents a later import. The desktop Lichess lane and PC service retain shared readers, bounded caches/deadlines and rate-limit cooldowns; malformed replies cannot become misses. The common picker labels identities/ratings, rejects stale choices and bounds its keyboard-scrollable dropdown to the viewport. Both checkouts passed 42 frontend tests, 35 service tests and 18 real-component groups at 1100/760/360px and 100/200% scale; the retained baseline failed 12 groups. Isolated TypeScript, lint and production/worker builds passed. Primary TypeScript still reports the pre-existing numeric elapsedMs test versus modified bigint binding mismatch; concurrent bindings were preserved. No native collection, owner data mutation, app restart, phone deployment or release. The subsequent profile milestone records the separate modal changes. See docs/FIDE_LOOKUP_RECOVERY.md.


- On 2026-09-12, adapter 80 / live pipeline 85 recovers quiet pawn-fork preparations by proving the original guard could capture the premature pawn advance, then covering every accepting/declining reply. Connected recovery includes actual countercaptures, checks and promotions; arbitrary first checks or passed-pawn advances cannot finance a fork. Standalone proofs use 16,384 visits and at most five further attacking moves; preparations share the existing 8,192-visit budget. Root preparation, actual-ply fork/check, board arrows and missed-opportunity review agree, while unproved causal prevention stays neutral. Existing countercapture/discovery noise filters remain. Only one of 159 full source/live course results changes; the other 158, all 63 positional exercises and 32 frozen priorities are unchanged, not certified accuracy successes. Verification: 141 fresh engine searches, 1,297 selected tests, 444 controller-worker inputs, builds, lint, three service tests and two dev-cache scenarios pass. Public worker median/p95/max is 71/213/464 ms excluding engine/UI; cold HTTP first/next startup is 7,803/85 ms. See benchmarks/tactical-relevance/private-course-review.md and the private adapter80-human-review.md for contrary evidence, exact local-vs-engine values and bounded-proof limits. The unrelated OTB type error, broader quiet/rare/causal gaps and native verification remain open. No app/package/service was restarted or deployed; paid data stays private.

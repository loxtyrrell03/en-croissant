...
- On 2026-09-08, approved Stats A phone source dbfb9b72567163bd7fe6b0ae6de32ac8e8436dd1 was published from the clean release checkout. The publish health check found no local home-server listener or process; the existing scheduled launcher started the stopped server. Local and public HTTPS health now identify that commit. Served phone Stats navigation verified the visible performance graph, matching graph endpoint/headline, collapsed/open comparison, and 20-rated/10-unrated filtering with synthetic provider data at 390px; controls measured 16px/45px, with no page errors or horizontal overflow. The Outpost development identity confirms the primary checkout and serves the new hero/graph/comparison module. Native desktop processes were not restarted, and physical-device interaction was not performed.

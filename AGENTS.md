...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-9` and live pipeline 9. Mistake attribution now checks the opponent's first reply in the better move's own legal engine continuation, not just whether the identical refutation still works. Locally proved net material signatures track the same target pieces across choices and can match different replies, capturing pieces or motifs. Equal-valued different victims, a smaller baseline gain, illegal lines and incomplete proofs do not establish persistent loss; the actual named-target capture is exchange-settled so a proof's lower bound cannot hide a larger observed loss. A fresh Stockfish control exposes the need: h6 allows Re8+ pinning Qf8, while best Kg7 still allows Rf5+ discovered check winning that same queen for a rook. The 67 cp difference is not explained by creating that already-forced queen loss, so the card retains the genuine motifs as tactical danger instead of blaming the move for it. Headline and saved timeline metadata agree. Double-check mates now receive the same immediate king-escape comparison as named mates. The shared desktop/phone foundation retains one primary lesson, conditional both-side/repeated ply evidence, strict discovered-ray geometry, actual promotion plies, dead-position stops, losing-alternative filtering, bounded quiet mate proofs, and the 4,096-node extra-check discovery proof. Conditional ray threats remain root-only, medium confidence, explicitly non-guaranteed and gated at -30 cp. Versioned idle migration and the rebuilt shared-review worker carry the adapter. Verification passed 149 focused adapter/review/persistence/phone/render tests, 32 fresh Stockfish 18 depth-16 judgement scenarios plus the restricted faster-mate check, targeted lint, the shared-review worker build, and the 8,851-module production Vite build. Type checking retains the unrelated `otbGameImport.test.ts` number/bigint fixture error. `benchmarks/tactical-relevance/README.md` and five JSON reports document judged lessons, rejected premises, counterfactual controls and limits; these small constructed sets are not general accuracy estimates. The broader tuning goal remains active: first-reply material comparisons do not cover all later tactical/positional differences, longer combinations, defensive resources, candidate coverage, overlapping named-mate descriptions or running-app proof. The vendored website v55 primitive classifier is unchanged. No runtime or hosting deployment is asserted.

...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-5` and live pipeline 5. The shared desktop/phone adapter legally replays lines, rejects poisoned captures and capturable geometric forks, and limits main themes to the immediate forcing sequence. Verified quiet mating preparations now pass that cutoff: a hypothetical pass only proposes a mate threat, then every legal defence must allow mate next turn; supplied mate-in-three candidates get a two-level all-defences tree with legal attacking alternatives. The searches have 4,096/16,384-node caps and bounded caches; exhausted proofs are withheld. Short checking/capturing mating PVs use the same verification, so a cooperative mate does not establish a forced one. Live Tactics excludes losing alternatives, shows one main headline/board label, and keeps both-side/repeated motifs in separate expandable ply timelines. Routine recaptures, generic opening/check tags, and incidental pins are suppressed. A quiet preparation is the main cause and its final mating pattern stays secondary. Mistake Review compares immediate refutations after played/best moves using legality, exchange, fork-target, and mate witnesses; persistent danger alone is not blamed on the mistake. Do not replay the old full refutation under changed defence. Metadata retains actor, comparison evidence, and separate timelines; idle migration uses the new version and the shared-review worker was rebuilt. Verification passed 112 focused adapter/review/persistence/phone/render tests, nine fresh Stockfish 18 depth-16/MultiPV3 position judgements, four before/after judgements, two quiet-mating judgements plus a restricted faster-mate search, targeted lint, the shared-review worker build, and the 8,851-module production Vite build. Whole-project type checking still reaches the unrelated `otbGameImport.test.ts` number/bigint fixture error. `benchmarks/tactical-relevance/README.md` and three JSON reports record evidence and limitations, including O-O preventing the f7 fork, Nxe4 saving the knight while winning the queen, and Qh2's quiet mating preparation. The unrestricted engine omitted a faster Qh6 mate in the constructed quiet position; a restricted search and local proof confirmed it, so candidate coverage remains separate from classification correctness. These small judgement sets are not general accuracy estimates. The broader tuning goal remains active: non-mating quiet preparations, longer mates, defensive combinations, pawn breakthroughs, candidate coverage, and deeper best-move comparisons need more evidence; unproven zugzwang is withheld. The vendored website v55 primitive classifier is unchanged. No running-app visual proof or hosting deployment is asserted.

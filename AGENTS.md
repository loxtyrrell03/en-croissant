...
- Full Moves engine deployment verified (2026-09-12): `e92598befd4bccdd5666562701530c32c5028c64` is served at the established `lox-pc` phone origin, with matching app/runtime versions. The live browser passes the same three-line depth-10, settings, tab continuity, played-move, pause and 320/390/768px checks, plus the active-service-worker stale-status regression. Both backends were healthy during verification. The services' observed Off choice was restored afterwards and the updated phone shell remains available while Off. This documentation-only follow-up does not replace that release; physical iPhone rendering remains unverified.

- Moves-tab engine restoration (2026-09-12): the user requires the full engine in Moves. `BoardWorkspace` now uses the existing full engine panel in both Moves and Engine, retaining one mounted engine instance across tabs; other board tools keep their existing presentation. Evaluations, all selected analysis lines, start/pause and engine settings are directly available above the move list. Seventeen existing engine/workspace tests and the production build pass. The preview harness verifies three real depth-10 PC lines, settings, tab-switch continuity, playing an engine move and pausing at 320/390/768px without page errors or horizontal overflow. Publication/live confirmation follows separately. Preserve the service switch state after temporary verification; no physical-phone interaction is claimed.

- Exact-address deployment verified (2026-09-12): release `73061f1dcb05c825bbcf7416ab5e5708f60547c5` is live at the restored `https://lox-pc.tail89d19b.ts.net/`, with matching app-version/runtime metadata, review retrieval, both services Ready, and hidden service processes. The real browser harness fails on the preceding release when seeded with stale cached Off state, then passes on this release with the service worker active and no page errors: fresh status plus actual engine start/analysis responses and visible depth 10. Its interactive worker releases afterwards. The build passes; only the original root Serve handler changed from 8787 to controller 8786, and the device IP is unchanged. This documentation/QA follow-up does not replace that verified release. Physical-phone confirmation is pending.

- Phone address correction (2026-09-12): the user's installed app opens `https://lox-pc.tail89d19b.ts.net/`; verifying only the replacement Windows hostname missed its DNS failure. Restored the persistent Tailscale hostname to `lox-pc` and routed that root through controller 8786, preserving other Serve handlers and the device IP. The exact original HTTPS address now resolves and a rendered phone-sized app displays real Stockfish depth-10 analysis. Keep this established hostname stable; discovering a renamed hostname during publication does not migrate installed PWAs or their origin-scoped saved data. The offline worker also bypasses all API/v1 and explicit no-store GETs so cached Ready/Off/review values cannot impersonate live services. Four executable cache regressions and four existing PWA tests pass; three new regressions failed before the fix. `scripts/qa-phone-engine.mjs --url <exact-phone-address>` additionally checks real wake/analysis traffic and stale cached service state with the service worker enabled. Cache-fix deployment verification follows separately; no physical iPhone interaction is claimed.

- Final installed release verification (2026-09-12): phone assets and home runtime both report source `a18c2dcf05244cc3417e779d007a8a18a4c21888`; current Tailscale HTTPS health, review retrieval and service status all pass with services enabled and one controller. All four installed controller scripts match source. A real HTTPS Stockfish analysis reached depth 10 and its interactive worker was released afterwards. The saved archive/analysis hashes remain unchanged. This documentation-only follow-up does not replace the verified runtime release. Reboot and physical iPhone interaction remain untested.

- Final Windows recovery refinement: controller replacement explicitly reclaims only the installed PS/Node identities because stopping WScript alone leaves children behind. Service helpers use hidden, uncaptured stdio and direct exit receipts; the controller runner uses `WaitForExit()` rather than `Start-Process -Wait`, which waits on surviving descendants. Windows' configured failure retry did not recover a killed controller in the actual 80-second audit, so independent `EnCroissantHomeServerWatchdog` checks every minute and restarts a stopped controller (two missed checks for a hung running task). The final watchdog automatically recovered an intentionally killed controller in 29.26 seconds; a home-server crash recovered in 8.11 seconds. The live 320/390/768px phone test turned Off, reloaded the independently served app, and turned On to Ready in 16.44 seconds, with no page errors. The two original archive/analysis files remain byte-identical; only backendIdleMs changed in the engine configuration, and no service process had a visible window handle. These are installed-PC and headless-browser checks, not a physical iPhone or reboot test. Preserve the independent watchdog and persistent user Off choice.

- Live service-switch testing exposed the engine backend's separate 30-second self-exit timer. `backendIdleMs: 0` now keeps only the HTTP/UCI service alive; engine worker idle shutdown remains unchanged. Controller installation preserves the remaining engine configuration and stages the matching server entrypoint. Service POSTs acknowledge the persisted choice with 202, while polling reports Starting/Stopping until health confirms completion; concurrent opposite choices may coalesce before launch. Isolated real-process tests verify standby without engine workers and retain timed-exit compatibility.

- The service-recovery release integrates deployed phone commit `dbfb9b72` rather than rolling back its Stats A model/panel. Only the documentation merge overlapped; both records were retained. Service help now reuses the deployed `PerformanceHelp` component. Forty-nine focused model/control tests and the review-worker type check pass. The phone publisher now discovers current Tailscale DNS and private builds follow their opened origin.

- Phone service recovery (2026-09-12): `EnCroissantHomeServer` now supervises an independent loopback controller on 8786 using WScript and hidden PS/Node children; Task Scheduler tracks its lifetime, starts it at sign-in and retries controller failure. It proxies the existing home server on 8787 and checks both backends every five seconds. `/api/pc-services` persists a combined On/Off choice, serializes transitions, validates host/origin/JSON/client headers, and only controls the installed home/Stockfish identities. Off stops their current work but retains saved state and an independently served immutable phone shell; the engine task uses a hidden waiting wrapper. Startup is mutex-protected, and origins come from the installed controller configuration. Home phone builds now resolve every private API/engine/database/explorer consumer against the opened origin, avoiding stale baked-in Tailscale names. The compact phone switch shows verified state and causes engine/review consumers to retry after recovery. Backend failure/security/persistence tests, 27 focused URL/engine/control tests, launcher checks, type checking and production build pass; rendered checks cover on/off/retry/help and 320/390/768px. The installed live on/off/recovery audit follows separately. Keep unrelated dirty checkout work out of this isolated release.
...
- On 2026-09-08, phone Stats Overview and desktop Account Stats gained the same original dependency-free Bayesian True performance modules as Outpost. Account, time control and period use dropdowns; running strength, period performance, model ranges, score/colour/opening summaries and recent-game updates distinguish their meanings. Desktop imported-game analysis remains expandable; the outer workspace scrolls to the full dashboard. Phone Overview uses rated games, loads at most 5,000 games/ten years, and permits cancellation. Pre-game Lichess rating is separate from ratingDiff, Chess.com priors use PGN Elo headers, unfinished games are excluded, and archive URLs/failed exports remain visible errors. Existing engine-based Strength and descriptive form metrics retain their own behavior. Online settings are provisional and model-conditional; no untouched-test predictive improvement or true-strength interval calibration is claimed. See docs/TRUE_PERFORMANCE.md and the SVG prototypes produced before implementation. Verification: 77 focused model/adapter/report/strength tests, full type checking, production app/service builds, the release collector, and dedicated actual-phone/desktop headless fixture checks passed. Desktop QA injects explicit view-model fixtures without Tauri-global stubs. Portable build commands now resolve the installed Node CLI entrypoints directly after relocated package-manager wrappers were found pointing to a retired worktree. Runtime publication is recorded separately; physical iPhone/desktop interaction is not implied.

- True performance follow-up: cache identity now uses provider, normalized username and time control, and request effects explicitly track account fields. Retired Overview/chart helpers were removed; targeted lint is clean and all 77 focused tests/type checks pass. Initial phone release 7cc808f0 was published through the clean-source/ancestry/mutex guard and verified at the current Tailscale DNS name, including matching public health/version commits and a full served-app Stats navigation with fixture games, 16px/45px period controls, no overflow and no page errors. The previous hard-coded hostname no longer resolved; future publication must use the freshly observed Tailscale Self.DNSName. Run the unchanged guard under a PowerShell environment with standard utility modules available; the first legacy-shell attempt failed before release publication because Get-FileHash was unavailable. The normal desktop checkout passed 104 focused tests and a direct production frontend build; its whole-project type check retains the pre-existing OTB number/bigint test mismatch, outside this change. Desktop processes were retained. The following publication includes the cache/cleanup refinement.

- Final True performance deployment audit: source 226a3b0c5ec7f84a7f44f1316748d6a31f2d9fe2 is live through the guarded phone publisher. Public health and app-version source commits match, with app-shell SHA-256 216a761840930994dbfda692993974b5c3ad98d76dd7f464aa5fb1e6874a13f4; the source and installed OTB collector hash remains 91C2E2DF46E37C24EFDB441CCECAD7A11EFB895A9084956D39A667E1DEAB1BD2. A headless visit to the actual served app navigated into Stats and changed the period to Last 20 using intercepted synthetic provider games; period controls measured 16px text/45px height, with no page errors or horizontal overflow. This is served-app/browser evidence, not a physical iPhone test. Scoped code is synchronized into both maintained desktop checkouts without replacing unrelated work. Outpost's corresponding source is pushed and its normal development executable is rebuilt; existing desktop processes were not stopped. Online predictive superiority and calibrated true-strength coverage remain unproven, as stated in the UI and model documentation. This is a documentation-only audit of the deployed code.

- On 2026-09-08, Stats now honours Rated/Unrated/Both across shared fetch/cache/model/period paths. Phone Overview no longer forces rated games; its Lichess casual request explicitly selects rated=false. Empty desktop Stats links to Accounts; phone Add account focuses its existing Stats username field because phone has no global Accounts registry. Verified 45 focused tests, type checking, production build and headless desktop/phone dropdown and last-N checks. Primary and secondary desktop sources are synchronized. Physical phone interaction was not performed; deployment status is recorded separately.

- On 2026-09-08, The desktop unlinked-database empty state now exposes Add account to /accounts; previously its early return bypassed the shared empty-state action. Type checking and headless routing passed. Phone source 2a4f62c1 was published from a separate clean release checkout and verified on the served HTTPS Stats page with a deterministic 10-unrated-game sample, no overflow or page errors. This follow-up changes only desktop empty-state presentation; phone code remains that deployed version. Existing Tailscale streamer routes were retained. No physical phone interaction was performed.

- On 2026-09-08, ambiguous performance labels were replaced by Estimated playing strength (level after the latest game, showing all usable history count/date) and Performance in selected games (level shown by the chosen games, showing sample count/date range). Their short meanings remain visible; help explains earlier-history use versus selected-result evidence plus the starting account rating. Chart labels use Estimated strength consistently. The statistical calculations are unchanged. Type checks and rendered dropdown/help/layout checks passed at desktop and 360/390px widths; source was synchronized across Outpost and both En Croissant checkouts.

- On 2026-09-08, phone release 025d2662 published the clearer rating labels without restarting the shared server. Served-page checks confirmed both visible metric descriptions, game-type selection and narrow layout with no page errors or horizontal overflow. Outpost serves the same labels through its live development source; En Croissant desktop source is synchronized. This verifies browser delivery, not physical phone interaction.

- On 2026-09-08, Stats now implements approved option A with one selected-game performance headline, a visible cumulative graph, and Compare ratings collapsed by default. The graph dropdown also offers the earlier-history estimate; its default final point equals the headline. Selected completed results retain games missing rating data, and named openings replace raw ECO fallbacks. Period presets persist; All loaded games accurately labels bounded history. The shared portal help supports hover/focus/tap and Escape with viewport bounds. The source is synchronized into both maintained desktop checkouts and Outpost. Verification: 48 focused tests, type checking, production build, and actual desktop Account Stats/phone Stats headless dropdown, help, graph and narrow-layout checks passed. Phone keeps its existing native username setup; a global phone account registry remains a separate proposal. No physical-device interaction or native executable replacement is claimed; publication is recorded separately.

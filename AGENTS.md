...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-22` and live pipeline 22. A long mating PV no longer certifies its own endpoint: seven-or-more-ply claims require an all-legal-defences checking proof, bounded to seven attacking moves and 32,768 visits. The PV only orders attacking choices; unknown/exhausted searches abstain. Real 4Osgg now leads with Qf1+'s Forcing Mate, covering Kh3 and Kg3 after Qg1+ and their different mating moves. A White Rc1 control retains the cooperative source mate but permits Rxf1; the old ghost is rejected and fresh Stockfish finds Black losing. Root mating mechanisms retain priority, but later conditional attractions cannot displace the certified root attack. Material-attraction tags without a mating proof are withheld pending their own causal proof. Terminal mate labels are remapped to the actual mating ply and consolidated: one named pattern survives, conflicting names use factual Checkmate pending taxonomy adjudication, and repeated countdowns for the same certified attack are omitted. Concrete secondary mechanisms remain. En-passant evidence names the actual removed pawn and capture ply, fixing OezHt's bishop-move misattribution. Best-line comparison marks a certified equal-or-shorter mate persisting after both choices as existing danger; delayed mates are not silently equated. The all-development long-checking-mate audit selects by legal line shape, not source tags or output: 27 of 32 receive bounded proofs, all with root-ply primaries. 3kPn3, 8FLlO, IyijS, eimKB and hajE5 remain unresolved, not accepted negatives; this is family proof coverage, not general accuracy or holdout validation. Only 4Osgg's headline changes in the separate mixed 32-position expansion; terminal consolidation reduces its timeline entries from 51 to 40. Fourteen new regressions plus the opt-in corpus audit cover both colours, the omitted king defence, budgets, payoff anchors, en passant, root ranking and existing mating danger. Verification passed 261 selected tests in 26 files, all thirteen opt-in Stockfish tests, targeted formatting/lint, the shared desktop/phone review worker and 8,851-module production builds. Type checking retains only the unrelated OTB number/bigint fixture error. Reports and reproduction commands are in `benchmarks/tactical-relevance/README.md`. Prior all-reply/budget/identity safeguards for w1lKu's clearance and pin, G8wdr's shared-defender discovery and CSh8J's promotion-backed fork remain. Longer/quiet combinations, material attraction, named taxonomy, broad ordinary-game noise and actual runtime proof remain open. Website primitives, Outpost and live hosting/runtime are unchanged; no deployment or general accuracy claim is made.

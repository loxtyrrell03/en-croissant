...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-21` and live pipeline 21. Real w1lKu now leads with checking Clearance: Bh7+ clears d3 for Qb3, with Qf3 after Kf8 and Qh3 after Kh8. The proof checks every root reply and a quiet slider move through the genuinely vacated square, then at most four checks before mate/material payoff, sharing 32,768 visits. An equally good already-available forcing attack disqualifies the cleared route; the open-file rook control prevents borrowed causation. Capturing/non-checking clearances remain outside this fallback. Quiet preparation now recognizes stationary checking sliders and absolute pins whose removal makes a checking-piece recapture legal. It nominates a rook/queen payoff within 11 plies, uses two to four checks and retains its 16,384-visit bound and conditional engine gates. Mover identity must survive: another piece on its former square cannot supply participation. The source timeline creates Pin at ply 3 and exploits it for the fork at ply 5; evidence distinguishes created/existing pins and explains the illegal pawn recapture. Arrows show the actual king reply's queen route, or the pin alignment and knight move. The local lower bound is 100 cp, not the full engine evaluation. Ten new regressions cover both routes, Rxf7's four-check defence, timing/arrows, a queen-capturing bishop, an unrelated pawn move, independent rook attack and exhausted budgets. Five fresh branch diagnostics plus before/after tests cover the real Qxd4 mistake and missed Bh7+ after Bb1. Only w1lKu's headline changes in the frozen 32-position sample. Previous invariants remain: immediate proved lessons outrank conditional later material motifs; mating consequences retain priority; G8wdr's shared-defender discovery and CSh8J's promotion-backed fork retain all-reply, named-target and budget controls. Verification passed 247 selected regression tests in 25 files, all twelve opt-in Stockfish tests, targeted formatting/lint, the shared desktop/phone review worker and 8,851-module production builds. Type checking retains only the unrelated OTB number/bigint fixture error. `benchmarks/tactical-relevance/README.md`, `expanded-review.md` and refreshed reports document behaviour and limits. Unresolved en-passant/long combinations, mate taxonomy, broad candidate coverage and actual runtime proof remain open. Website primitives, Outpost and live hosting/runtime are unchanged; no general accuracy or deployment claim is made.

...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-8` and live pipeline 8. Discovered attack/check/double-check labels now require a genuinely vacated blocker on an unchanged slider's ray and a proved gain or short mate; they are reconstructed at the causal ply rather than inherited from a later PV endpoint. One canonical discovery label replaces overlapping aliases, with factual square/piece explanations and board arrows for the opened ray and accompanying attack. Immediate material proofs include checking-line interpositions; a bounded 4,096-node AND/OR extension checks every legal defence and at most one extra checking move before a named-target capture with legal exchange settlement. Incomplete proofs are withheld. Fresh engine evidence corrected a proposed negative f7-pawn example: f6 Rxf6 Qxf6 Bxf6 wins queen and pawn for rook, while the alternative Kg8 requires Rg5+ and a queen block. Candidate controls separately reject Bf3 allowing Qxe1# and Rf5+ without a material target; unrestricted scans still find the superior Bxh7+ and Re8# respectively. The shared desktop/phone foundation retains one primary lesson, conditional both-side/repeated ply evidence, played/best-choice comparisons for immediate captures/forks/mates/pins/skewers/defender removal, target relocation including castling, dead-position stops, actual promotion plies, losing-alternative filtering, and bounded quiet mate proofs. Engine-supported ray threats with checking replies remain root-only, medium confidence, non-guaranteed and gated at -30 cp. Versioned idle migration and the rebuilt shared-review worker carry the adapter. Verification passed 143 focused adapter/review/persistence/phone/render tests, 29 fresh Stockfish 18 depth-16 judgement scenarios plus the restricted faster-mate check, targeted lint, the shared-review worker build, and the 8,851-module production Vite build. Whole-project and review-worker type checking still reaches the unrelated `otbGameImport.test.ts` number/bigint fixture error. `benchmarks/tactical-relevance/README.md` and five JSON reports record judgements, unrestricted versus selected candidates, rejected premises and limits; these small constructed sets are not general accuracy estimates. The broader tuning goal remains active: longer combinations, defensive resources, candidate coverage, deeper best-move comparisons, overlapping named-mate descriptions, and running-app proof remain incomplete. The vendored website v55 primitive classifier is unchanged. No runtime or hosting deployment is asserted.

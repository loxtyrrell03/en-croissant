...
- On 2026-09-08, tactical relevance gained a first causal-audit milestone (`site-55.adapter-3`, live pipeline 3). The shared desktop/phone adapter legally replays each line, rejects poisoned captures and capturable geometric forks using legal exchange/reply checks, and stops importing themes after the attacking side leaves the immediate forcing sequence. Live Tactics uses root-side evaluations to exclude losing alternatives or alternatives that miss a proven mate, shows one main headline/board label from the best line, and keeps supporting themes in expandable ply rows. Mistake Review uses one primary lesson, compares the scale of allowed versus missed payoffs, and preserves secondary evidence and its ply; the phone card builder uses the same primary selector. Saved motif metadata retains relevance/payoff fields and the version change triggers existing idle migration. The generated shared-review worker was rebuilt. Verification passed 89 focused adapter/review/persistence/phone tests, nine fresh Stockfish 18 depth-16/MultiPV3 position judgements, targeted lint, both worker builds, and the 8,851-module production Vite build. Whole-project and review-worker type checking still reach the unrelated `otbGameImport.test.ts` number/bigint fixture error. `benchmarks/tactical-relevance/README.md` and its recorded engine report document the judgements, including removal of starting-position clearance, drawn-ending zugzwang, and labels from losing alternatives. This is a progress milestone: the broader tuning goal remains active. Quiet preparations, pure mate threats, defensive combinations, pawn breakthroughs, best-move counterfactuals, and repeated/both-side motifs at every ply still need stronger judgement/proof coverage; unproven zugzwang is withheld. The vendored website v55 primitive classifier is unchanged. No running-app visual proof or hosting deployment is asserted.

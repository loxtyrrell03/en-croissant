...
- On 2026-09-12, adapter 86 / live pipeline 91 verifies new quiet mating threats as bounded material-or-mate attacks (8,192 shared operations, three further checks), then proves checking guard deflection separately (4,096 operations). In the real Lichess case, Mating Attack leads, the actual later Deflection stays secondary and its capture says Deflection Payoff; the board does not borrow future capture arrows. Missed lessons retain the root certificate while unsupported causal comparisons stay neutral. Deferred-capture and pin proofs now debit off-square liabilities; a stronger verified checking discovery subsumes only its own initial capture badge. One rare root is recovered and four course continuations lose unsafe claims, change mechanism or lower their local bound. All 180 course primary labels, 176 full results, other nineteen rare results, 32 priorities and 152 prior public headlines are unchanged, not certified accurate. Ninety fresh engine searches retain contrary controls and distinguish local bounds from full-position evaluations. Verification: 1,476 selected tests, 532 production-worker inputs, builds, lint, whole-project type checking, three service tests and two dev-cache scenarios pass. Public worker median/p95/max is 75/230/628 ms excluding engine/UI; six cold HTTP cases pass with variable multi-second startup, not native latency proof. See benchmarks/tactical-relevance/quiet-mating-attack-review.md and private-course-review.md for limits and receipts. Existing interference/exact KPK coverage remains; drawing/larger-ending zugzwangs and broader quiet/rare/causal/native gaps stay open. The earlier unrelated OTB type error is absent in this checkout without a tactical change to it. No app/package/service was restarted or deployed; paid data stays private.

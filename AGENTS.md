...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-14` and live pipeline 14. Trapped-piece labels are reconstructed from a named victim and every legal defence, requiring extra recapture-adjusted material beyond the initiating capture. No safe victim flight is allowed. A new defender must demonstrably improve the target exchange in a protection-only removal probe, then be capturable or subject to one verified quiet absolute pin; unrelated captures/promotions cannot earn that pin's gain. Proofs cap reply/candidate visits at 256 and pin proofs at eight, abstaining on exhaustion without reusing larger-budget successes. Real 6mAvx now prioritizes trapped Ra1 over the bishop capture and explicitly answers Qc3 with Bb4; open-rank, missing-bishop and castled-king controls withhold the trap. A larger initial queen win remains primary over the extra rook. Real 8mguL now proves the trapped rook's exchange-sacrifice minimum of 170 cp. Trap labels/arrows name the actual piece, duplicate pin/trap badges are suppressed, and mistake comparisons can prove persistence or identify a legal safe flight under a different supplied choice. A fixed twelve-game diagnostic selection from the website's Lichess development fixture now has source-line and fresh unrestricted/root-restricted Stockfish reports; source tags select cases but are not expected headlines, and no holdout rows were selected. Real Bb4 and c7 interference false negatives are recovered by considering attacks on the defender, blocker-as-attacker relationships, legal promotion captures and immediate recapture-adjusted promotions by the blocking pawn. Board arrows include the attack on that defender. Independently verified knight forks no longer lend their gain to incidental non-king discovered rays. Clearance must name another piece's tactical use of the vacated square within the connected episode and does not automatically claim a sacrifice; this necessary condition is not a complete forcing proof. The diagnostic fixture records unresolved Bxd5/Bxf4 defender-removal combinations, quiet Rb6 skewer preparation, intermediate-capture explanations and pawn-ending proof boundaries. Do not turn these known misses into successful empty-result assertions. Material interference is reconstructed at the blocking move from a real pre-existing sliding defence, a protection-only blocker/defender-removal probe, and every legal defensive reply. The named target, an attacked defender, the piece taking the blocker or the blocking pawn's immediate promotion must yield positive recapture-adjusted material; cooperative PVs, pseudo-defenders already pinned, unrelated gains and incomplete proofs cannot qualify. Board arrows stop the defensive ray at the blocker and show the target attack. Four new fresh-engine controls distinguish supported e7+ (a drawing resource, not a winning position), safe removal of an unsupported blocker, losing quiet e7 versus sound d7+, and an existing pin that actually enables Qxh7. Removing-the-defender labels cannot outrank a capture already earning the entire proved gain or borrow an unrelated mate flag; the last control now correctly prioritizes the pin. Timeline labels stop after two consecutive quiet plies; forced check evasions and locally verified mating preparations do not count as quiet pauses. This bounds relevance, not legal engine-line visibility or assessment when navigating to a later position. Deferred captures of pieces still on a square where they earned approximately their own value (including promotion gain) are exchange continuations, not newly hanging pieces; moving again clears that credit and a queen capturing only a pawn does not qualify. Same-ply discovered attacks/checks suppress redundant clearance labels. Fresh f7 controls retain Nxf7's fork, queen payoff and Black's bishop capture without labelling the knight's subsequent recapture as a new blunder; Bxf7+ retains immediate weak-f7 evidence without distant post-development badges. Material deflection labels are reconstructed from a captured bait, the departing defender and an immediate named-target payoff. A protection-only restoration probe must show that the defender's departure worsens the legal exchange; already pinned pseudo-defenders do not qualify. Every real defensive reply must allow the target gain or verified mate-in-one, with a 4,096-move cap on optional mating answers. Incomplete proofs abstain, and unsupported non-capturing/delayed deflections must not regain endpoint-only acceptance. Board arrows show the defender's departure and target capture. Fresh engine controls distinguish Re8+ Rxe8 Qxd5 from a safe Kh7 refusal, and reveal Raxe8# as the stronger lesson when an extra rook already pins the alleged queen defender. A non-king discovered attack no longer borrows an independent mate's value; explicit local material values are preserved and the mating lesson outranks that side-effect. Discovered checks and verified king-attraction branches retain their own mating evidence. The shared desktop/phone foundation still has one primary lesson, conditional both-side/repeated ply evidence, actual promotion plies, dead-position stops, losing-alternative filtering, bounded quiet-mate/discovery proofs and played/best-choice comparisons. Those comparisons can match different replies and motifs winning the same named target, but cannot equate different victims, smaller gains or incomplete proofs; no old PV is transplanted under another move. Conditional ray threats remain root-only, medium confidence, explicitly non-guaranteed and gated at -30 cp. Versioned idle migration and the rebuilt shared-review worker carry the adapter. Verification passed 192 focused adapter/review/persistence/phone/render tests, the 39 prior fresh Stockfish 18 depth-16 judgement scenarios, five trap/defending-resource controls, a twelve-position real-puzzle diagnostic audit with targeted acceptance assertions and the restricted faster-mate check, targeted lint, the shared-review worker build, and the 8,851-module production Vite build. Type checking retains the unrelated `otbGameImport.test.ts` number/bigint fixture error. `benchmarks/tactical-relevance/README.md` and nine JSON reports plus the real-puzzle diagnostic fixture document judged lessons, rejected premises, counterfactual controls and limits; these small constructed and development-selected sets are not general accuracy estimates; report generation does not certify every diagnostic lesson. The broader tuning goal remains active: local exchange proofs do not cover arbitrary continuations or longer trap-defence sequences, non-capturing/delayed deflections, long quiet combinations across the relevance boundary, purely mating defender removal, delayed/mating interference, all sacrifice/self-interference/intermezzo claims, positional compensation, candidate coverage, overlapping named-mate descriptions or running-app proof. The vendored website v55 primitive classifier is unchanged. No runtime or hosting deployment is asserted.

...
- On 2026-08-31, OTB cross-source deduplication began treating terminal DGT noise as one game when target side, normalized players, result, and date agree: mainlines must share at least 12 exact plies and may then be an exact prefix or disagree only within the final two plies of each recording. Earlier or broader divergences remain separate. A supplied real tournament copy ending `59...d3` versus a broadcast tail of `60.Ra4 Ke5 61.Ra1` is pinned as a regression alongside a different-final-move boundary test. The same bounded rule is present in Outpost. Both products passed all 27 focused OTB Rust tests, and the headless En Croissant collector passed `cargo check`. No running desktop process was restarted, no existing library row was changed, and the phone collector source has not yet been published to the installed home-server runtime.

...
- On 2026-09-14, adapter 105 / live pipeline 110 distinguishes a comparable capture choice from an established missed-material cause. Only immediate generic hanging-piece lessons qualify, using two legal positive capture-square exchanges; a locally comparable or larger played capture does not prove equal overall safety/value. Actual-ply timelines, desktop/phone wording, saved metadata and shared-service reload preserve the qualification; independently supported opponent root lessons retain priority. The shared PGN reader now assigns move colour from the legal turn, fixing Black-started FEN imports and variations without changing local ply indices. Twenty-one fixed boards from four real games span openings, quiet positions, exchanges and endings; 127 fresh engine searches support the review. One misleading cause is qualified; the connected follow-up is still unproved. All 246 private full results, twenty rare results, 32 priorities and 808 prior public worker headlines are unchanged, not certified accurate. Verification: 2,092 selected tests, 1,379 controller-worker inputs, twelve actual React/Chrome groups, 81 cold-HTTP cases, builds, TypeScript, scoped lint, four service and two cache tests. The broader PGN run retains one unrelated stats wording failure, reproduced with the old reader. Public worker median/p95/max is 65/210/878 ms over 871 inputs excluding engine/UI; HTTP startup remains variable up to 3,941 ms. See benchmarks/tactical-relevance/capture-choice-relevance-review.md and private-course-review.md for receipts, prior quiet-mate work and remaining connected-cause/nature/endgame/native gaps. No owner app/package/service restart or deployment; existing stored imports/cards are not rewritten and paid data stays private.

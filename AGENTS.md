...
- On 2026-09-08, tactical relevance advanced to `site-55.adapter-7` and live pipeline 7. The shared desktop/phone adapter compares pin, skewer and defender-removal replies after played/best choices, tracks relocated target pieces (including castling's rook), and records a concrete legal defence or persistent danger. It never transplants the full old PV under a changed choice; ambiguous identity or incomplete exchange proof cannot establish prevention. Headline and timeline metadata retain the same comparison. Material proofs distinguish guaranteed captures from specific ray threats where every non-checking reply concedes material but quiet checking replies remain. The latter require an engine-evaluated root score of at least -30 cp, use medium confidence and explicit non-guarantee wording, and cannot leak into unevaluated later rows. Mating counterchecks and material-winning capture/promotion replies do not qualify as harmless delays. Fresh before/after evidence shows h6 allowing a pin-based drawing resource, while Kd7 permits Kxd6; other cases find Kf4 allowing Qxd3 against the skewer and Qxd1+ preventing defender removal by forcing White to answer check. The foundation remains one primary lesson, conditional both-side/repeated ply evidence, dead-position timeline stops, actual-ply promotion labels, board relationships from legal fork/pin/skewer geometry, losing-alternative filtering, and bounded quiet mate proofs (4,096/16,384 nodes). Versioned idle migration and the rebuilt shared-review worker carry the adapter. Verification passed 133 focused adapter/review/persistence/phone/render tests, 23 fresh Stockfish 18 depth-16 judgement scenarios (nine original positions, seven before/after, two quiet preparations, five material candidates), a restricted faster-mate check, targeted lint, the shared-review worker build, and the 8,851-module production Vite build. Whole-project type checking still reaches the unrelated `otbGameImport.test.ts` number/bigint fixture error. `benchmarks/tactical-relevance/README.md` and four JSON reports document evidence, rejected premises and limits; small judgement sets are not general accuracy estimates. Engine-supported checking-tempo threats are not exhaustive long-line proofs, and candidate selection previously omitted a faster mate confirmed separately. The broader tuning goal remains active: longer combinations, defensive resources, candidate coverage, deeper best-move comparisons, and running-app proof remain incomplete. The vendored website v55 primitive classifier is unchanged. No runtime or hosting deployment is asserted.

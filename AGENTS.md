...
- On 2026-08-30, the phone Import surface stopped using a portal-positioned FIDE dropdown on the web companion. Phone searches now render a bounded native touch-scroll list inline, so scrolling either the results or page cannot dismiss the choices; every suggestion is a full-width 56 px target. Import text fields and actions use the 16 px/touch-sized Mantine tier that prevents iOS focus zoom, and FIDE ID/year stack below the player field at phone widths. The shared desktop picker retains its normal combobox because only the PC-hosted phone caller opts into inline results. The root manifest now has an explicit app identity/start/scope and the shell carries Apple standalone/title/status-bar/icon metadata. This removes Safari's URL controls only when the site is installed and launched from the iPhone Home Screen; an ordinary Safari tab cannot hide browser-owned chrome. Regression verification passed the six focused picker/PWA/OTB identity tests, TypeScript checking, targeted formatting/lint, the OTB-prep build, and the canonical-host production Vite build. The broader web tests retained 171 passes and one unrelated baseline engine-detail wording failure; the full suite retained 646 passes and three unrelated baseline failures in that wording, audio mocking, and timestamp expectations. The first deployment exposed only a verifier defect: four unused source fallbacks still embedded the retired `gaming-pc` host even though configured requests used `lox-pc`; every phone fallback now names the canonical root so a missing build variable cannot route hosted databases, cloud evals, engines, or shared Lichess credentials to the obsolete host. This milestone remains in deployment verification until a matching fallback-clean public bundle and canonical URL proof are recorded.

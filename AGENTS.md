...
- On 2026-09-12, adapter 85 / live pipeline 90 adds independently proved, branch-specific Mating Clearance as a supporting lesson. Every legal defence must be mated within three further checking attacking moves under 4,096 shared operations; a different mating branch cannot borrow the cleared route. Root-only mates are independently proved, while actual-ply Discovered Check/Double Check keep their more precise labels without duplicate clearance. Forcing Mate remains primary; its board shows the trigger and cleared route, preserves the support on preview, and suppresses nearby same-rank badge overlap. Announced fifty-move claims now guard the affected mate verifiers before future capture resets. One real Lichess and three course supporting explanations change; all 180 course primary labels, 177 full course results, other 19 rare results, 32 priorities and 146 prior public headlines remain unchanged, not certified accurate. Fifty-one final fresh engine searches retain contrary branches and faster constructed alternatives. Verification: 1,423 selected tests, 525 production-worker inputs, lint, builds, three service and two dev-cache tests pass. Public worker median/p95/max is 83/238/610 ms excluding engine/UI; five HTTP cases pass but first/KPK startup is 4,660/5,190 ms, not a startup fix. See benchmarks/tactical-relevance/mating-clearance-review.md and private-course-review.md for proof scope and receipts. Existing interference and exact KPK zugzwang coverage is retained; drawing/larger-ending zugzwangs, broader quiet/rare/causal gaps, the unrelated OTB type error and native verification remain open. No app/package/service was restarted or deployed; paid data stays private.

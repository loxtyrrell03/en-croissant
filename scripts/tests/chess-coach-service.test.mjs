...
test("stats digest includes opponent bands, transparent quality comparisons, and position outcomes", () => {
      ...
      findings: [
        {
          ...
          confidence: "medium",
          games: 8,
          title: "Clock pressure is converting into errors",
          detail: "Heavy-pressure games score 37.5% with 8.5 errors per 100 moves.",
        },
      ],
    },
  });
  ...
  assert.match(
    digest,
    /CLOCK_PRESSURE_SEGMENT\|type=Heavy pressure\|games=8\|record=2W\/2D\/4L\|score=37\.5%/,
  );
  assert.match(digest, /DERIVED_COACH_FINDING\|tone=risk\|confidence=medium\|games=8/);
});

test("stats report accepts the unified all-source and all-speed scope", () => {
  const payload = normalizeStatsReportRequestPayload({
    requestId: "stats-all-scope",
    periodLabel: "Last 365 days",
    source: "combined",
    username: "Loxi-ty + lachlan1415",
    timeClass: "all",
    aggregate: { record: { games: 4367 } },
  });
  assert.equal(payload.source, "combined");
  assert.equal(payload.timeClass, "all");
  assert.equal(payload.username, "Loxi-ty + lachlan1415");
});

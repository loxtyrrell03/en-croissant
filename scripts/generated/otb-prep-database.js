...
var r$1 = class {
	...
	map(r, t) {
		return this._chain((t) => n$1.ok(r(t)), (r) => n$1.err(t ? t(r) : r));
	}
	chain(r, t) {
		return this._chain(r, t || ((r) => n$1.err(r)));
	}
};
...
var Position = class {
	...
	validate() {
		if (this.board.occupied.isEmpty()) return n$1.err(new PositionError(IllegalSetup.Empty));
		if (this.board.king.size() !== 2) return n$1.err(new PositionError(IllegalSetup.Kings));
		if (!defined(this.board.kingOf(this.turn))) return n$1.err(new PositionError(IllegalSetup.Kings));
		const otherKing = this.board.kingOf(opposite(this.turn));
		if (!defined(otherKing)) return n$1.err(new PositionError(IllegalSetup.Kings));
		if (this.kingAttackers(otherKing, this.turn, this.board.occupied).nonEmpty()) return n$1.err(new PositionError(IllegalSetup.OppositeCheck));
		if (SquareSet.backranks().intersects(this.board.pawn)) return n$1.err(new PositionError(IllegalSetup.PawnsOnBackrank));
		return n$1.ok(void 0);
	}
	dropDests(_ctx) {
		return SquareSet.empty();
	}
...
var parseBoardFen = (boardPart) => {
	...
	for (let i = 0; i < boardPart.length; i++) {
		...
		if (c === "/" && file === 8) {
			...
			if (step > 0) file += step;
				...
				if (file >= 8 || rank < 0) return n$1.err(new FenError(InvalidFen.Board));
				const square = file + rank * 8;
				const piece = charToPiece(c);
				if (!piece) return n$1.err(new FenError(InvalidFen.Board));
				if (boardPart[i + 1] === "~") {
					piece.promoted = true;
					i++;
				}
	...
	if (rank !== 0 || file !== 8) return n$1.err(new FenError(InvalidFen.Board));
	return n$1.ok(board);
};
var parsePockets = (pocketPart) => {
	if (pocketPart.length > 64) return n$1.err(new FenError(InvalidFen.Pockets));
	const pockets = Material.empty();
	for (const c of pocketPart) {
		const piece = charToPiece(c);
		if (!piece) return n$1.err(new FenError(InvalidFen.Pockets));
		pockets[piece.color][piece.role]++;
	}
	return n$1.ok(pockets);
};
var parseCastlingFen = (board, castlingPart) => {
	let castlingRights = SquareSet.empty();
	if (castlingPart === "-") return n$1.ok(castlingRights);
	for (const c of castlingPart) {
		...
		} else return n$1.err(new FenError(InvalidFen.Castling));
	}
	if (COLORS.some((color) => SquareSet.backrank(color).intersect(castlingRights).size() > 2)) return n$1.err(new FenError(InvalidFen.Castling));
	return n$1.ok(castlingRights);
};
var parseRemainingChecks = (part) => {
	...
	if (parts.length === 3 && parts[0] === "") {
		...
		if (!defined(white) || white > 3 || !defined(black) || black > 3) return n$1.err(new FenError(InvalidFen.RemainingChecks));
		return n$1.ok(new RemainingChecks(3 - white, 3 - black));
	} else if (parts.length === 2) {
		const white = parseSmallUint(parts[0]);
		const black = parseSmallUint(parts[1]);
		if (!defined(white) || white > 3 || !defined(black) || black > 3) return n$1.err(new FenError(InvalidFen.RemainingChecks));
		return n$1.ok(new RemainingChecks(white, black));
	} else return n$1.err(new FenError(InvalidFen.RemainingChecks));
};
var parseFen = (fen) => {
	const parts = fen.split(/[\s_]+/);
	const boardPart = parts.shift();
	let board;
	let pockets = n$1.ok(void 0);
	if (boardPart.endsWith("]")) {
		const pocketStart = boardPart.indexOf("[");
		if (pocketStart === -1) return n$1.err(new FenError(InvalidFen.Fen));
	...
	else return n$1.err(new FenError(InvalidFen.Turn));
	return board.chain((board) => {
		const castlingPart = parts.shift();
		const castlingRights = defined(castlingPart) ? parseCastlingFen(board, castlingPart) : n$1.ok(SquareSet.empty());
		const epPart = parts.shift();
		let epSquare;
		if (defined(epPart) && epPart !== "-") {
			epSquare = parseSquare(epPart);
			if (!defined(epSquare)) return n$1.err(new FenError(InvalidFen.EpSquare));
		}
		...
		if (!defined(halfmoves)) return n$1.err(new FenError(InvalidFen.Halfmoves));
		const fullmovesPart = parts.shift();
		const fullmoves = defined(fullmovesPart) ? parseSmallUint(fullmovesPart) : 1;
		if (!defined(fullmoves)) return n$1.err(new FenError(InvalidFen.Fullmoves));
		const remainingChecksPart = parts.shift();
		let remainingChecks = n$1.ok(void 0);
		if (defined(remainingChecksPart)) {
			if (defined(earlyRemainingChecks)) return n$1.err(new FenError(InvalidFen.RemainingChecks));
			remainingChecks = parseRemainingChecks(remainingChecksPart);
		} else if (defined(earlyRemainingChecks)) remainingChecks = earlyRemainingChecks;
		if (parts.length > 0) return n$1.err(new FenError(InvalidFen.Fen));
...
var Crazyhouse = class extends Position {
	...
	validate() {
		return super.validate().chain((_) => {
			var _a, _b;
			if ((_a = this.pockets) === null || _a === void 0 ? void 0 : _a.count("king")) return n$1.err(new PositionError(IllegalSetup.Kings));
			if ((((_b = this.pockets) === null || _b === void 0 ? void 0 : _b.size()) || 0) + this.board.occupied.size() > 64) return n$1.err(new PositionError(IllegalSetup.Variant));
			return n$1.ok(void 0);
		});
	}
	hasInsufficientMaterial(color) {
...
var RacingKings = class extends Position {
	...
	validate() {
		if (this.isCheck() || this.board.pawn.nonEmpty()) return n$1.err(new PositionError(IllegalSetup.Variant));
...
var Horde = class extends Position {
	...
	validate() {
		if (this.board.occupied.isEmpty()) return n$1.err(new PositionError(IllegalSetup.Empty));
		if (this.board.king.size() !== 1) return n$1.err(new PositionError(IllegalSetup.Kings));
		const otherKing = this.board.kingOf(opposite(this.turn));
		if (defined(otherKing) && this.kingAttackers(otherKing, this.turn, this.board.occupied).nonEmpty()) return n$1.err(new PositionError(IllegalSetup.OppositeCheck));
		for (const color of COLORS) {
			const backranks = this.board.pieces(color, "king").isEmpty() ? SquareSet.backrank(opposite(color)) : SquareSet.backranks();
			if (this.board.pieces(color, "pawn").intersects(backranks)) return n$1.err(new PositionError(IllegalSetup.PawnsOnBackrank));
		}
		return n$1.ok(void 0);
	}
	hasInsufficientMaterial(color) {
...
var startingPosition = (headers) => {
	...
	if (!rules) return n$1.err(new PositionError(IllegalSetup.Variant));
	const fen = headers.get("FEN");
	if (fen) return parseFen(fen).chain((setup) => setupPosition(rules, setup));
	else return n$1.ok(defaultPosition(rules));
};
...
//#endregion
//#region scripts/otb-prep-database.ts
function buildWebOtbPrepDatabase({ name, pgn, importedAt }) {
...
//#endregion
export { buildWebOtbPrepDatabase };

...
const STATS_TIME_CLASSES = new Set(["all", "bullet", "blitz", "rapid", "classical", "daily"]);
  ...
  return (Array.isArray(openings) ? openings : [])
    ...
    .map((opening) =>
      statsLine(tag, [
        ...
        statsField("analyzed_games", statsFigure(opening?.analyzedGames)),
        statsField("analysis_coverage", statsPercent(opening?.analysisCoveragePct)),
        statsField("engine_accuracy", statsPercent(opening?.avgAccuracy, 1)),
        statsField("engine_acpl", statsFigure(opening?.avgAcpl, 1)),
        statsField("errors_per_100_moves", statsFigure(opening?.errorsPer100Moves, 2)),
      ]),
    );
}
...
export function formatStatsAggregateDigest(aggregate) {
  ...
  const lines = [
    ...
    ...(openings?.bestAccuracy
      ? statsOpeningLines("OPENING_CLEANEST_ENGINE", [openings.bestAccuracy])
      : []),
    ...(openings?.weakestAccuracy
      ? statsOpeningLines("OPENING_ENGINE_GAP", [openings.weakestAccuracy])
      : []),
    mistakes
      ? statsLine("MISTAKES", [
    ...
    statsLine("COACH_EVIDENCE_COVERAGE", [
      statsField("analyzed_games", statsFigure(coaching?.analyzedGames)),
      statsField("analysis_coverage", statsPercent(coaching?.analysisCoveragePct, 1)),
    ]),
    ...["clockPressure", "movePace", "gameTypes"].flatMap((group) =>
      (Array.isArray(coaching?.[group]) ? coaching[group] : []).map((segment) =>
        statsLine(
          group === "clockPressure"
            ? "CLOCK_PRESSURE_SEGMENT"
            : group === "movePace"
              ? "MOVE_PACE_SEGMENT"
              : "GAME_TYPE_SEGMENT",
          [
            statsField("type", oneLine(segment?.label, 40)),
            statsField("games", statsFigure(segment?.games)),
            statsField(
              "record",
              `${statsFigure(segment?.wins)}W/${statsFigure(segment?.draws)}D/${statsFigure(segment?.losses)}L`,
            ),
            statsField("score", statsPercent(segment?.scorePct, 1)),
            statsField("accuracy", statsPercent(segment?.avgAccuracy, 1)),
            statsField("acpl", statsFigure(segment?.avgAcpl, 1)),
            statsField("errors_per_100_moves", statsFigure(segment?.errorsPer100Moves, 2)),
            statsField("engine_complexity", statsFigure(segment?.avgComplexity, 2)),
            statsField("confidence", oneLine(segment?.confidence, 12)),
          ],
        ),
      ),
    ),
    ...(Array.isArray(coaching?.findings) ? coaching.findings : []).map((finding) =>
      statsLine("DERIVED_COACH_FINDING", [
        statsField("tone", oneLine(finding?.tone, 12)),
        statsField("confidence", oneLine(finding?.confidence, 12)),
        statsField("games", statsFigure(finding?.games)),
        statsField("title", oneLine(finding?.title, 120)),
        statsField("evidence", oneLine(finding?.detail, 500)),
      ]),
    ),
  ];
...
- Name the actual openings and cite the actual figures (records, engine accuracy, error rates, rating points, seconds) from the digest. Distinguish an opening's result score from its move accuracy.
...
- Use CLOCK_PRESSURE_SEGMENT and MOVE_PACE_SEGMENT to explain how time use correlates with both score and engine error rate. Describe association, not causation.
- GAME_TYPE_SEGMENT is an engine-volatility classification: positional-leaning means quieter evaluation movement and tactical-leaning means sharper evaluation movement. Present it as a measured tendency, not a claim that every position in the game had that character.
- Prefer medium/high-confidence findings. If a useful finding is low confidence, state the sample size and frame it as a lead to monitor, not a settled weakness.

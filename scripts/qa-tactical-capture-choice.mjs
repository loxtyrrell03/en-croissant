...
const root = process.cwd(),
  output = resolve(
    root,
    compensatedMode ? "tmp/mistake-compensated-adapter116" : pawnMode ? "tmp/mistake-pawn-adapter115" : missedAlternativeMode ? "tmp/mistake-alternative-adapter114" : alternativeMode ? "tmp/mistake-alternative-adapter113" : discoveryMode ? "tmp/mistake-discovery-adapter107" : natureMode ? "tmp/mistake-nature-v4" : "tmp/tactical-capture-choice-adapter105",
  );
...
import{compensatedCaptureReviewInput}from'@/utils/tests/fixtures/compensatedCapture';
...
const samples={
compensated:compensatedCaptureReviewInput,
...
if(restore){const migrateMotifs=['discovery','alternative','missedAlternative','pawn','compensated'].includes(which);value.mistakeReview=positionSchema.shape.mistakeReview.parse(JSON.parse(JSON.stringify({...value.mistakeReview,nature:'tactical',natureClassifierVersion:which==='discovery'?4:3,...(migrateMotifs?{motifClassifierVersion:'site-55.adapter-106',natureReason:'Old explanation'}:{})})));const migrated=migrateMotifs?await migrateMistakeReviewDeckMotifClassifications({positions:[value]}):await migrateMistakeReviewDeckNatureClassifications({positions:[value]});value.mistakeReview=migrated.deck.positions[0].mistakeReview;}
...
try {
  ...
  for (const which of compensatedMode ? ["compensated"] : pawnMode ? ["pawn"] : missedAlternativeMode ? ["missedAlternative", "alternative"] : alternativeMode ? ["alternative"] : discoveryMode ? ["discovery"] : natureMode ? ["capture", "quiet", "fork"] : ["capture"])
    for (const width of [1100, 760, 360])
      for (const scale of [1, 2])
        for (const restore of [false, true]) {
          ...
          if (which === "compensated") {
            await page.getByText("What you missed: Material Gain", {exact:true}).waitFor();
          ...
          } else if (which === "missedAlternative") {
            await page.getByText("Missed alternative: Hanging Piece", {exact:true}).waitFor();
            await page.getByText("Alternative move you missed", {exact:true}).click();
            const detail = page.locator("details").filter({has:page.getByText("Alternative move you missed", {exact:true})});
          ...
          assert(!(await page.locator("main").innerText()).includes("Alternative move you missed"), "No missed alternative before reveal");

...
export function preparePrivatePgnSample(source, count = 24) {
  ...
  const size = Math.min(count, positions.length);
  const indices = Array.from({ length: size }, (_, i) =>
    size === 1 ? 0 : Math.floor((i * (positions.length - 1)) / (size - 1)),
  );
  return {
    selection:
    ...
    cases: indices.map((index) => ({
      id: `private-easy:${index + 1}`,
      eligibleIndex: index + 1,
      ...positions[index],
    })),
  };
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (!input || !output)
    throw new Error(
      "Usage: node scripts/benchmarks/private-pgn-sample.mjs INPUT_PGN PRIVATE_OUTPUT_JSON [COUNT]",
    );
  const report = preparePrivatePgnSample(readFileSync(input, "utf8"), size ? Number(size) : 24);

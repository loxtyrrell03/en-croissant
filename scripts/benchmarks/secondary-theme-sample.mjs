...
const [source, output, frozenExclusions] = process.argv.slice(2);
...
const seed = "secondary-theme-broad-2026-09-14";
const rows = raw
  .trim()
  .split(/\r?\n/)
...
for (const theme of themes) {
  ...
  for (const { row, hash } of selected) {
    ...
    if (cases.filter((row) => row.stratum === theme).length === 3) break;
  }
  if (cases.filter((row) => row.stratum === theme).length !== 3)
  ...
  JSON.stringify(
    {
      ...
      seed,
      selection:
        "First three SHA-256 ordered unseen development rows per stratum, excluding prior source games and IDs. Distinct games across strata. Selection precedes chess judgement, engine analysis and classifier output.",

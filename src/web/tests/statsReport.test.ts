...
describe("computePeriodReport", () => {
    ...
    test("joins engine quality to openings and correlates pressure and game type with results", () => {
        const games = Array.from({ length: 10 }, (_, index) =>
            makeGame({
                end: sec(7, 10, 8, index),
        ...
        const entries = games.map((game, index): AnalyzedGameEntry => {
            const clean = index < 5;
            return {
                ...makeAnalyzedEntry(game, {
                    accuracy: clean ? 92 : 68,
                    acpl: clean ? 22 : 88,
                    counts: clean
                ...
                stats: {
                    ...
                    acpl: clean ? 22 : 88,
                    scoredCount: 20,
                    complexity: clean ? 3 : 7,
                    fastRate: clean ? 0.2 : 0.75,
                    scramble: clean ? 0 : 0.25,
                },
            };
        });
        const report = computePeriodReport({
            games,
            analyzed: entries,
            windowStart: sec(7, 10),
            windowEnd: sec(7, 11) - 1,
            label: "One day",
            nowSec: sec(7, 10, 20),
            currentRating: 1500,
        });

        expect(report.openings.bestAccuracy).toMatchObject({
            name: "Italian Game",
            analyzedGames: 5,
            avgAccuracy: 92,
        });
        expect(report.openings.weakestAccuracy).toMatchObject({
            name: "Sicilian Defense",
            analyzedGames: 5,
            avgAccuracy: 68,
        });
        expect(report.coaching.clockPressure).toEqual(
            expect.arrayContaining([
                ...
                expect.objectContaining({
                    ...
                    games: 5,
                    scorePct: 0,
                    errorsPer100Moves: 15,
                }),
            ]),
        );
        expect(report.coaching.gameTypes).toEqual(
            expect.arrayContaining([
                expect.objectContaining({ key: "positional", avgAccuracy: 92 }),
                expect.objectContaining({ key: "tactical", avgAccuracy: 68 }),
            ]),
        );
        expect(report.coaching.findings.map((finding) => finding.title)).toEqual(
            expect.arrayContaining([
                "Sicilian Defense is the clearest opening gap",
                "Clock pressure is converting into errors",
                "Quiet positions suit you better",
            ]),
        );
    });
    ...
    test("keeps mixed-source insights while suppressing incomparable rating statistics", () => {
        const chesscom = makeGame({
            end: sec(7, 6, 10),
            source: "chesscom",
            rating: 1500,
            providerQuality: {
        ...
        const report = computePeriodReport({
            games: [chesscom, lichess],
            analyzed: [],
            windowStart: sec(7, 1),
            windowEnd: sec(7, 31) - 1,
            label: "July",
            nowSec: sec(7, 30),
            currentRating: null,
            ratingsComparable: false,
        });

        expect(report.record).toMatchObject({ games: 2, wins: 1, losses: 1 });
        expect(report.perf).toBeNull();
        expect(report.rating).toEqual({ start: null, end: null, delta: null });
        expect(report.form).toBeNull();
        expect(report.providerQuality).toBeNull();
        expect(report.weekly.every((week) => week.perf === null && week.ratingEnd === null)).toBe(
            true,
        );
    });
});

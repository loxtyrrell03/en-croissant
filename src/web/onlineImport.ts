...
export async function fetchWebOnlineGames({
    ...
    return source === "chesscom"
        ? fetchChessComGames(trimmedUsername, limit, since, onProgress, signal)
        : fetchLichessGames(trimmedUsername, limit, since, onProgress, signal);
}
...
async function fetchLichessGames(
    ...
    const games = (await response.text())
        ...
        .filter(
            (game): game is { id?: string; pgn: string; lastMoveAt?: number; createdAt?: number } =>
        ...
        .map((game) => ({
            ...
            url: game.id
                ? `https://lichess.org/${game.id.slice(0, 8)}`

...
type StatsSourceFilter = StatsSource | "combined";
type StatsTimeClassFilter = StatsTimeClass | "all";
type StatsSettings = {
  insightsVersion: 1;
  source: StatsSourceFilter;
  chesscomUsername: string;
  lichessUsername: string;
  timeClass: StatsTimeClassFilter;
...
const STATS_TIME_CLASSES: StatsTimeClassFilter[] = [
  "all",
  "bullet",
  "blitz",
  "rapid",
...
function normalizeStatsSettings(
  ...
  const useUnifiedArchiveDefault = value?.insightsVersion !== 1;
  const source: StatsSourceFilter =
    useUnifiedArchiveDefault || value?.source === "combined"
      ? "combined"
      : value?.source === "lichess"
        ? "lichess"
        : "chesscom";
  let timeClass: StatsTimeClassFilter = STATS_TIME_CLASSES.includes(
    value?.timeClass as StatsTimeClassFilter,
  )
    ? (value?.timeClass as StatsTimeClassFilter)
    : "all";
  if (useUnifiedArchiveDefault) timeClass = "all";
  ...
  return {
    ...
    period: isStatsPeriodKey(value?.period) ? value.period : "365",
    customStart: normalizeDateInput(value?.customStart) || formatDateInput(monthAgo),
    customEnd: normalizeDateInput(value?.customEnd) || formatDateInput(today),
...
function getStatsSourceLabel(source: StatsSourceFilter) {
...
function StatsOverviewSection({
  ...
  const performance = useMemo(
    () =>
      ratingsComparable
        ...
        : null,
    [currentRating, games, nowSec, ratingsComparable, windowStart],
  );
  const form = useMemo(
  ...
  const ratedGames = useMemo(
    () =>
      ratingsComparable ? games.filter((game) => game.rated && Number.isFinite(game.rating)) : [],
    [games, ratingsComparable],
  );
  ...
  return (
    <Box className={styles.cardGrid}>
      <Box className={styles.card}>
        ...
        {performance ? (
          ...
          <Text className={styles.emptyLine}>
            {ratingsComparable
              ? "Not enough games."
              : "Choose one source and speed for comparable ratings."}
          </Text>
        )}
      </Box>
      ...
      <Box className={styles.card}>
        ...
        {!ratingsComparable ? (
          <Text className={styles.emptyLine}>Choose one source and speed for rating history.</Text>
        ) : ratedFilter === "casual" ? (
...
function StatsStrengthSection({
  ...
  const periodEntries = useMemo(() => {
    const selectedKeys = new Set(games.map(gameAnalysisKey));
    return entries.filter(
      (entry) =>
        selectedKeys.has(entry.key) &&
        entry.end >= periodWindow.start &&
        entry.end <= periodWindow.end,
    );
  }, [entries, games, periodWindow.end, periodWindow.start]);
...
function StatsSegmentTable({
  title,
  rows,
  note,
}: {
  title: string;
  rows: PeriodReport["coaching"]["clockPressure"];
  note: string;
}) {
  if (rows.length === 0) return null;
  return (
    <div className={styles.segmentBlock}>
      ...
      <div className={styles.segmentTable}>
        ...
        {rows.map((row) => (
          <div className={styles.segmentRow} key={row.key}>
            <div>
              <strong>{row.label}</strong>
            ...
            <span>{row.games}</span>
            <strong>{Math.round(row.scorePct)}%</strong>
            <span>{formatStatsAccuracy(row.avgAccuracy)}</span>
            <span>{formatStatsDecimal(row.errorsPer100Moves)}</span>
          </div>
        ))}
      </div>
      <Text size="xs" c="dimmed" mt={4}>
...
function StatsCoachingInsightsCard({ report }: { report: PeriodReport }) {
  const coaching = report.coaching;
  return (
    <Box className={`${styles.card} ${styles.cardWide}`}>
      <div className={styles.sectionHead}>
        <div>
          <span className={styles.cardLabel}>Improvement insights</span>
          <Text size="xs" c="dimmed">
            Results joined to the standardized engine analysis for the same games
          </Text>
        </div>
        <Badge variant="light" color="blue" size="sm">
          {coaching.analyzedGames}/{report.record.games} ·{" "}
          {Math.round(coaching.analysisCoveragePct)}%
        </Badge>
      </div>

      {coaching.findings.length > 0 ? (
        <div className={styles.findingGrid}>
          {coaching.findings.map((finding) => (
            <Box
              className={styles.aiCard}
              data-tone={
                finding.tone === "strength" ? "good" : finding.tone === "risk" ? "bad" : "focus"
              }
            ...
            >
              <Group justify="space-between" gap="xs" wrap="nowrap">
                ...
                <Badge variant="transparent" color="gray" size="xs">
                  {finding.confidence}
                </Badge>
              </Group>
              <Text size="xs" mt={2}>
        ...
        <Text className={styles.emptyLine}>
          Insights appear as this period gains at least five analyzed games in comparable groups.
        </Text>
      )}

      <div className={styles.segmentGrid}>
        <StatsSegmentTable
          title="Clock pressure → results and errors"
          rows={coaching.clockPressure}
          note="Pressure is the share of moves played with under 12% of the starting clock. This is correlation, not causation."
        />
        <StatsSegmentTable
          title="Move pace → results and errors"
          rows={coaching.movePace}
          note="Snap rate groups premoves and decisions below the calibrated fast-move threshold."
        />
        <StatsSegmentTable
          title="Game type → results and errors"
          rows={coaching.gameTypes}
          note="Positional/tactical labels come from Stockfish evaluation volatility: quieter versus sharper games."
        />
      </div>
    </Box>
  );
...
function StatsAiReportCard({
  ...
  // Period reports do not need the optional chess-book corpus used by the
  // per-game coach, so model availability is sufficient here.
  const coachReady = Boolean(health?.modelAvailable);
  ...
  return (
    <Box className={`${styles.card} ${styles.cardWide}`}>
      ...
      <Textarea
        ...
        placeholder="Ask about this period: opening gaps, time management, tactical vs positional games..."
        aria-label="Question for the stats coach"
      />
      {cached ? (
        <Group justify="flex-end" mt={6}>
          <Button
            ...
            onClick={() => void generateReport()}
          >
            {question.trim() ? "Ask about this period" : "Refresh report"}
          </Button>
        </Group>
      ) : null}
      ...
      {cached ? (
        ...
        <Box mt="sm">
          <Button
            ...
            onClick={() => void generateReport()}
          >
            {question.trim() ? "Ask coach" : "Generate AI report"}
          </Button>
        </Box>
      )}
    </Box>
...
function StatsReportSection({
  ...
  const report = useMemo(
    () =>
      computePeriodReport({
        ...
        nowSec,
        currentRating,
        ratingsComparable: source !== "combined" && timeClass !== "all",
      }),
    [
      analyzed,
      currentRating,
  ...
  return (
    <Box className={styles.cardGrid}>
      ...
      <Box className={styles.card}>
          ...
          <div className={styles.openingRows}>
            {openingRows.map((opening) => (
              <div className={styles.openingRow} key={opening.key}>
                ...
                <div className={styles.openingMetrics}>
                  <strong>{Math.round(opening.scorePct)}% score</strong>
                  <small>
                    {opening.avgAccuracy != null
                      ? `${opening.avgAccuracy.toFixed(1)}% acc · ${opening.analyzedGames}a`
                      : "not analyzed"}
                  </small>
                </div>
              </div>
        ...
        {report.openings.best || report.openings.worst ? (
          <div className={styles.openingCallouts}>
            ...
            {report.openings.bestAccuracy ? (
              <Text size="xs">
                <Text component="span" size="xs" c="blue.3" fw={700}>
                  Cleanest:
                </Text>{" "}
                {report.openings.bestAccuracy.name}{" "}
                {report.openings.bestAccuracy.avgAccuracy?.toFixed(1)}% accuracy (
                {report.openings.bestAccuracy.analyzedGames} games)
              </Text>
            ) : null}
            {report.openings.weakestAccuracy ? (
              <Text size="xs">
                <Text component="span" size="xs" c="orange.3" fw={700}>
                  Accuracy gap:
                </Text>{" "}
                {report.openings.weakestAccuracy.name}{" "}
                {report.openings.weakestAccuracy.avgAccuracy?.toFixed(1)}% accuracy (
                {report.openings.weakestAccuracy.analyzedGames} games)
              </Text>
            ) : null}
          </div>
        ) : null}
      ...
      <StatsCoachingInsightsCard report={report} />
...
export default function StatsWorkspace({ lichessToken = "" }: { lichessToken?: string }) {
  ...
  useEffect(() => {
    const controller = new AbortController();
    void loadPcStatsConfig(controller.signal)
      .then((config) => {
        if (!config) return;
        const patch: Partial<StatsSettings> = {};
        if (!settings.chesscomUsername.trim() && config.accounts.chesscom) {
          patch.chesscomUsername = config.accounts.chesscom;
        }
        if (!settings.lichessUsername.trim() && config.accounts.lichess) {
          patch.lichessUsername = config.accounts.lichess;
        }
        if (Object.keys(patch).length) updateSettings(patch);
      })
      .catch(() => undefined);
    return () => controller.abort();
  }, [settings.chesscomUsername, settings.lichessUsername, updateSettings]);

  const activeUsername =
    settings.source === "combined"
      ? [settings.chesscomUsername, settings.lichessUsername].filter(Boolean).join(" + ")
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      void configurePcStatsSync({
        accounts: {
          chesscom: settings.chesscomUsername.trim(),
          lichess: settings.lichessUsername.trim(),
        },
        historyDays: 365,
        depth: 25,
        nodesPerPosition: 1_000_000,
      }).catch(() => undefined);
    }, 500);
  ...
  const ratingsComparable = settings.source !== "combined" && settings.timeClass !== "all";
  const usesPcArchive = settings.source === "combined" || settings.timeClass === "all";
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    const gameRequest = usesPcArchive
      ? loadPcStatsGames(controller.signal).then((storedGames) => {
          const cutoff = Math.floor(Date.now() / 1000) - requestedDays * 86400 - 86400;
          return storedGames
            .filter((game) => settings.source === "combined" || game.source === settings.source)
            .filter((game) => settings.timeClass === "all" || game.timeClass === settings.timeClass)
            .filter(
              (game) =>
                settings.rated === "both" ||
                (settings.rated === "rated" ? game.rated : !game.rated),
            )
            .filter((game) => game.end >= cutoff)
            .sort((left, right) => left.end - right.end)
            .slice(-STATS_MAX_GAMES);
        })
      : fetchStatsGames({
          source: settings.source as StatsSource,
          username: trimmedUsername,
          timeClass: settings.timeClass as StatsTimeClass,
    ...
    void gameRequest
      .then((games) => {
        ...
        const series = ratingsComparable ? computePerformanceSeries(games, { windowSize: 20 }) : [];
  ...
  }, [ratingsComparable, refreshKey, settings.source, settings.timeClass, trimmedUsername]);
  ...
  useEffect(() => {
    if (!trimmedUsername || settings.source === "combined") {
      setPoolRatings({});
      return;
    }
    const controller = new AbortController();
  ...
  const periodWindow = useMemo(
    ...
    [nowSec, settings.customEnd, settings.customStart, settings.period],
  );
  ...
  const currentRating = useMemo(() => {
    if (!ratingsComparable) return null;
  ...
  }, [data, fetchedRating, periodGames, ratingsComparable]);

  const commitUsername = useCallback(() => {
    if (settings.source === "combined") return;
  ...
  const timeClassOptions = useMemo(() => {
    ...
    return settings.source === "chesscom"
      ? [...common, { value: "daily", label: "Daily" }]
      : settings.source === "lichess"
        ? [...common, { value: "classical", label: "Classical" }]
        : [
            ...common,
      ...
      <Box className={`${classes.panel} ${styles.controlsCard}`}>
        <Group gap="xs" wrap="nowrap" className={styles.controlsRow}>
          <SegmentedControl
            ...
            onChange={(value) =>
              updateSettings({
                source:
                  value === "combined" ? "combined" : value === "lichess" ? "lichess" : "chesscom",
              })
            }
            data={[
          ...
          <TextInput
            ...
            value={usernameDraft}
            disabled={settings.source === "combined"}
        ...
        <div className={styles.selectRow}>
          <Select
            ...
            onChange={(value) =>
              value && updateSettings({ timeClass: value as StatsTimeClassFilter })
            }
        ...
        {settings.period === "custom" ? (
          <div className={styles.customRangeRow}>
            <TextInput
              ...
              min={formatDateInput(new Date((nowSec - 364 * 86400) * 1000))}
              max={settings.customEnd}
              onChange={(event) => updateSettings({ customStart: event.currentTarget.value })}
            />
            <TextInput
              size="xs"
              type="date"
              ...
              min={settings.customStart}
              max={formatDateInput(new Date(nowSec * 1000))}
              onChange={(event) => updateSettings({ customEnd: event.currentTarget.value })}
            />
          </div>
        ) : null}
        {data ? (
      ...
      {!trimmedUsername ? (
        ...
        settings.tab === "overview" ? (
          <StatsOverviewSection
            ...
            ratedFilter={settings.rated}
            ratingsComparable={ratingsComparable}

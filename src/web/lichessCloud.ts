...
export async function queryWebLichessCloudEngineMoves({
    ...
    const requestedMultipv = Math.max(
        1,
        Math.min(12, Math.round((multipv ?? uniqueMoves.length) || 1)),
    );
    ...
    const missingMoves = uniqueMoves
        .filter((move) => !bySan.has(move))
        .slice(0, MAX_CHILD_EVAL_MOVES);
...
export async function queryWebLichessCloudLines({
    ...
    return webLichessCloudDataToLines(fen, data, "lichess-cloud");
}

export function webLichessCloudDataToLines(
    fen: string,
    data: WebLichessCloudData,
    source: "lichess-cloud" | "lichess-cloud-stored" = "lichess-cloud",
): WebEngineLine[] {
    return (data.pvs ?? [])
        .map((pv, index) => {

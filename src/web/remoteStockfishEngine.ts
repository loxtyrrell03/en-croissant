...
export async function queryRemoteStoredCloudLines({
    ...
    const url = new URL(`${baseUrl.replace(/\/$/, "")}/v1/cloud-eval`);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Stored cloud eval lookup returned ${response.status}${detail ? `: ${detail}` : ""}`,
            );
        }
        const data = (await response.json()) as WebLichessCloudData;
        return webLichessCloudDataToLines(fen, data, "lichess-cloud-stored");
    } catch (error) {
        if (connectTimedOut) throw new Error("Gaming PC stored cloud evals are unreachable.");
        throw error;
    } finally {

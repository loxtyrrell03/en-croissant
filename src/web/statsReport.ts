...
export type AnalysisSegment = PatternSplit & {
    avgAccuracy: number | null;
    avgAcpl: number | null;
    errorsPer100Moves: number | null;
...
export type PeriodReport = {
    ...
    openings: {
        ...
        bestAccuracy: OpeningAgg | null;
        weakestAccuracy: OpeningAgg | null;
    };
    mistakes: {
        analyzedGames: number;
        avgAccuracy: number | null;
    ...
    coaching: {
        analyzedGames: number;
        analysisCoveragePct: number;
        clockPressure: AnalysisSegment[];
        movePace: AnalysisSegment[];
        gameTypes: AnalysisSegment[];
...
const OPENING_QUALITY_MIN_GAMES = 5;
...
export function computePeriodReport(input: {
    ...
    ratingsComparable?: boolean;
}): PeriodReport {
    const { analyzed, currentRating, label, nowSec, windowEnd, windowStart } = input;
    const ratingsComparable = input.ratingsComparable !== false;
    const games = input.games
        .filter((game) => game.end >= windowStart && game.end <= windowEnd)
        .sort((a, b) => a.end - b.end);
    const openings = computeOpenings(games, analyzed);
    const mistakes = computeMistakes(games, analyzed, currentRating);

    return {
        window: { start: windowStart, end: windowEnd, label },
        record: computeRecord(games),
        perf: ratingsComparable
            ...
            : null,
        rating: ratingsComparable
            ? computeRatingSpan(games)
            : { start: null, end: null, delta: null },
        form:
            ratingsComparable && games.length > 0
        ...
        mistakes,
        providerQuality:
            new Set(games.map((game) => game.source)).size <= 1
                ? computeProviderQuality(games)
                : null,
        opponents: computeOpponents(games, analyzed, currentRating),
        highlights: computeHighlights(games),
        patterns: computePatterns(games),
        coaching: computeCoachingSummary(games, analyzed, openings, mistakes),
        weekly: computeWeekly(games, {
            windowStart,
            windowEnd,
            nowSec,
            currentRating,
...
function computeOpenings(
    games: StatsGame[],
    analyzed: AnalyzedGameEntry[],
): PeriodReport["openings"] {
    type OpeningWork = OpeningAgg & {
    ...
    const groups = new Map<string, OpeningWork>();
    for (const game of games) {
        ...
        if (entry) {
            agg.analyzedGames += 1;
            const moves = Math.max(0, entry.stats.scoredCount);
            agg.scoredMoves += moves;
            agg.errors += entry.counts.mistake + entry.counts.blunder;
            if (entry.stats.accuracy !== null && Number.isFinite(entry.stats.accuracy)) {
                agg.accuracyTotal += entry.stats.accuracy * moves;
                agg.accuracyMoves += moves;
            }
            if (entry.stats.acpl !== null && Number.isFinite(entry.stats.acpl)) {
                agg.acplTotal += entry.stats.acpl * moves;
                agg.acplMoves += moves;
            }
        }
    }

    const all = Array.from(groups.values()).map((agg): OpeningAgg => {
        agg.scorePct = ((agg.wins + agg.draws * 0.5) / agg.games) * 100;
        agg.analysisCoveragePct = (agg.analyzedGames / agg.games) * 100;
        agg.avgAccuracy = agg.accuracyMoves > 0 ? agg.accuracyTotal / agg.accuracyMoves : null;
        agg.avgAcpl = agg.acplMoves > 0 ? agg.acplTotal / agg.acplMoves : null;
        agg.errorsPer100Moves = agg.scoredMoves > 0 ? (agg.errors / agg.scoredMoves) * 100 : null;
        const {
            accuracyTotal: _accuracyTotal,
            accuracyMoves: _accuracyMoves,
            acplTotal: _acplTotal,
            acplMoves: _acplMoves,
            errors: _errors,
            scoredMoves: _scoredMoves,
            ...result
        } = agg;
        return result;
    });
    ...
    const qualityQualifying = all.filter(
        (agg) => agg.analyzedGames >= OPENING_QUALITY_MIN_GAMES && agg.avgAccuracy !== null,
    );
    const bestAccuracy =
        [...qualityQualifying].sort(
            (a, b) =>
                (b.avgAccuracy ?? 0) - (a.avgAccuracy ?? 0) || b.analyzedGames - a.analyzedGames,
        )[0] ?? null;
    let weakestAccuracy: OpeningAgg | null =
        [...qualityQualifying].sort(
            (a, b) =>
                (a.avgAccuracy ?? 100) - (b.avgAccuracy ?? 100) ||
                b.analyzedGames - a.analyzedGames,
        )[0] ?? null;
    if (weakestAccuracy?.key === bestAccuracy?.key) weakestAccuracy = null;
...
function computeCoachingSummary(
    games: StatsGame[],
    analyzed: AnalyzedGameEntry[],
    openings: PeriodReport["openings"],
    mistakes: PeriodReport["mistakes"],
): PeriodReport["coaching"] {
    const entriesByKey = new Map(analyzed.map((entry) => [entry.key, entry]));
    ...
    const clockPressure = buildAnalysisSegments(matched, [
        {
            key: "low",
            label: "Low pressure",
            includes: (entry) => entry.stats.scramble !== null && entry.stats.scramble <= 0.02,
        },
        {
            key: "some",
            label: "Some pressure",
            includes: (entry) =>
                entry.stats.scramble !== null &&
                entry.stats.scramble > 0.02 &&
                entry.stats.scramble <= 0.12,
        },
        {
            key: "heavy",
            label: "Heavy pressure",
            includes: (entry) => entry.stats.scramble !== null && entry.stats.scramble > 0.12,
        },
    ]);
    const movePace = buildAnalysisSegments(matched, [
        {
            key: "deliberate",
            label: "Deliberate pace",
            includes: (entry) => entry.stats.fastRate !== null && entry.stats.fastRate <= 0.35,
        },
        {
            key: "typical",
            label: "Balanced pace",
            includes: (entry) =>
                entry.stats.fastRate !== null &&
                entry.stats.fastRate > 0.35 &&
                entry.stats.fastRate <= 0.6,
        },
        {
            key: "fast",
            label: "High snap rate",
            includes: (entry) => entry.stats.fastRate !== null && entry.stats.fastRate > 0.6,
        },
    ]);
    const gameTypes = buildAnalysisSegments(matched, [
        {
            key: "positional",
            label: "Positional-leaning",
            includes: (entry) => entry.stats.complexity !== null && entry.stats.complexity < 4,
        },
        {
            key: "mixed",
            label: "Mixed",
            includes: (entry) =>
                entry.stats.complexity !== null &&
                entry.stats.complexity >= 4 &&
                entry.stats.complexity < 6,
        },
        {
            key: "tactical",
            label: "Tactical-leaning",
            includes: (entry) => entry.stats.complexity !== null && entry.stats.complexity >= 6,
        },
    ]);

    const findings: CoachingFinding[] = [];
    const pushFinding = (finding: CoachingFinding | null) => {
        if (finding) findings.push(finding);
    };
    const bestOpening = openings.bestAccuracy;
    const weakOpening = openings.weakestAccuracy;
    if (
        bestOpening &&
        weakOpening &&
        bestOpening.avgAccuracy !== null &&
        weakOpening.avgAccuracy !== null
    ) {
        const gap = bestOpening.avgAccuracy - weakOpening.avgAccuracy;
        pushFinding({
            tone: "focus",
            title: `${weakOpening.name} is the clearest opening gap`,
            detail: `${weakOpening.avgAccuracy.toFixed(1)}% accuracy across ${weakOpening.analyzedGames} analyzed games, ${gap.toFixed(1)} points below ${bestOpening.name}. Its score was ${Math.round(weakOpening.scorePct)}%.`,
            confidence: confidenceForGames(weakOpening.analyzedGames),
            games: weakOpening.analyzedGames,
        });
        pushFinding({
            tone: "strength",
            title: `${bestOpening.name} produces your cleanest openings`,
            detail: `${bestOpening.avgAccuracy.toFixed(1)}% accuracy and a ${Math.round(bestOpening.scorePct)}% score across ${bestOpening.analyzedGames} analyzed games.`,
            confidence: confidenceForGames(bestOpening.analyzedGames),
            games: bestOpening.analyzedGames,
        });
    }

    pushFinding(
        compareSegmentsFinding(clockPressure, "low", "heavy", {
            positiveTitle: "You stay accurate under clock pressure",
            negativeTitle: "Clock pressure is converting into errors",
            neutralTitle: "Clock pressure changes your practical results",
            subject: "Heavy-pressure games",
        }),
    );
    pushFinding(
        compareSegmentsFinding(gameTypes, "positional", "tactical", {
            positiveTitle: "Sharp games suit you better",
            negativeTitle: "Quiet positions suit you better",
            neutralTitle: "Game type changes your results",
            subject: "Tactical-leaning games",
        }),
    );

    if (mistakes) {
        const phases = STRENGTH_PHASES.map((phase) => ({
            phase,
            ...mistakes.phaseQuality[phase],
        })).filter((row) => row.moves >= 20 && row.avgAccuracy !== null);
        const bestPhase = [...phases].sort(
            (a, b) => (b.avgAccuracy ?? 0) - (a.avgAccuracy ?? 0),
        )[0];
        const weakPhase = [...phases].sort(
            (a, b) => (a.avgAccuracy ?? 100) - (b.avgAccuracy ?? 100),
        )[0];
        if (
            bestPhase &&
            weakPhase &&
            bestPhase.phase !== weakPhase.phase &&
            bestPhase.avgAccuracy !== null &&
            weakPhase.avgAccuracy !== null
        ) {
            pushFinding({
                tone: "risk",
                title: `${titleCase(weakPhase.phase)} is your weakest phase`,
                detail: `${weakPhase.avgAccuracy.toFixed(1)}% accuracy across ${weakPhase.moves} moves, versus ${bestPhase.avgAccuracy.toFixed(1)}% in the ${bestPhase.phase}.`,
                confidence: confidenceForGames(mistakes.analyzedGames),
                games: mistakes.analyzedGames,
            });
        }
    }
...
function buildAnalysisSegments(
    matched: { game: StatsGame; entry: AnalyzedGameEntry }[],
    definitions: {
        key: string;
        label: string;
        includes: (entry: AnalyzedGameEntry) => boolean;
    }[],
): AnalysisSegment[] {
    return definitions.flatMap((definition): AnalysisSegment[] => {
        const rows = matched.filter(({ entry }) => definition.includes(entry));
        if (rows.length === 0) return [];
        const record = computeRecord(rows.map(({ game }) => game));
        let scoredMoves = 0;
        let accuracyTotal = 0;
        let accuracyMoves = 0;
        let acplTotal = 0;
        ...
        let complexityMoves = 0;
        let errors = 0;
        for (const { entry } of rows) {
            const moves = Math.max(0, entry.stats.scoredCount);
            scoredMoves += moves;
            errors += entry.counts.mistake + entry.counts.blunder;
            if (entry.stats.accuracy !== null && Number.isFinite(entry.stats.accuracy)) {
                accuracyTotal += entry.stats.accuracy * moves;
                accuracyMoves += moves;
            }
            if (entry.stats.acpl !== null && Number.isFinite(entry.stats.acpl)) {
                acplTotal += entry.stats.acpl * moves;
                acplMoves += moves;
            }
            if (entry.stats.complexity !== null && Number.isFinite(entry.stats.complexity)) {
                complexityTotal += entry.stats.complexity * moves;
                complexityMoves += moves;
            }
        }
        return [
            {
                ...
                avgAccuracy: accuracyMoves > 0 ? accuracyTotal / accuracyMoves : null,
                avgAcpl: acplMoves > 0 ? acplTotal / acplMoves : null,
                errorsPer100Moves: scoredMoves > 0 ? (errors / scoredMoves) * 100 : null,
                avgComplexity: complexityMoves > 0 ? complexityTotal / complexityMoves : null,
...
function compareSegmentsFinding(
    ...
    const baseline = segments.find((segment) => segment.key === baselineKey);
    const comparison = segments.find((segment) => segment.key === comparisonKey);
    if (!baseline || !comparison || baseline.games < 5 || comparison.games < 5) return null;
    const scoreDelta = comparison.scorePct - baseline.scorePct;
    const errorDelta =
        comparison.errorsPer100Moves !== null && baseline.errorsPer100Moves !== null
            ? comparison.errorsPer100Moves - baseline.errorsPer100Moves
            : null;
    const direction = scoreDelta >= 5 ? "positive" : scoreDelta <= -5 ? "negative" : "neutral";
    const title =
        direction === "positive"
            ? copy.positiveTitle
            : direction === "negative"
              ...
              : copy.neutralTitle;
    return {
        tone: direction === "positive" ? "strength" : direction === "negative" ? "risk" : "focus",
        title,
        detail: `${copy.subject} score ${Math.round(comparison.scorePct)}% versus ${Math.round(baseline.scorePct)}% in ${baseline.label.toLowerCase()} games (${formatSignedOne(scoreDelta)} pp).${errorDelta === null ? "" : ` Error rate changes by ${formatSignedOne(errorDelta)} per 100 moves.`}`,
        confidence: confidenceForGames(Math.min(baseline.games, comparison.games)),
        games: baseline.games + comparison.games,
    };
}

function confidenceForGames(games: number): "low" | "medium" | "high" {
    return games >= 20 ? "high" : games >= 8 ? "medium" : "low";
}
...
function computeWeekly(
    ...
    for (
        ...
        const performance = opts.ratingsComparable
            ? computePeriodPerformance(weekGames, {
        ...
        weeks.push({
            ...
            ratingEnd:
                opts.ratingsComparable && rated.length > 0 ? rated[rated.length - 1].rating : null,
        });
    }
    return weeks;
}

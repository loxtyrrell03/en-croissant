...
export default function OnlineGameAnalysisPanel({
  ...
  const [providerAccuracy, setProviderAccuracy] = useState(new Map<string, number>());
  ...
  useEffect(() => {
    ...
    void loadPcStatsConfig(controller.signal)
      .then((config) => {
        if (!config) return;
        if (!chessComUsername.trim() && config.accounts.chesscom) {
          setChessComUsername(config.accounts.chesscom);
        }
        if (!lichessUsername.trim() && config.accounts.lichess) {
          setLichessUsername(config.accounts.lichess);
        }
      })
      .catch(() => undefined);
    return () => controller.abort();
  }, [chessComUsername, lichessUsername, setChessComUsername, setLichessUsername]);
  ...
  useEffect(() => {
    let active = true;
    const refresh = () => {
      void Promise.all([loadPcAnalyzedEntries(), loadPcProviderAccuracies()])
        .then(([entries, provider]) => {
          if (active) {
            setAnalyzedEntries(entries);
            setProviderAccuracy(provider);
          }
        })
        .catch(() => undefined);
    };
    refresh();
    const timer = games.length > 0 ? window.setInterval(refresh, 15_000) : null;
    return () => {
      active = false;
  ...
  const analyzedAccuracy = useMemo(
    () => mergeAccuracyMaps(providerAccuracy, accuracyByGameUrl(analyzedEntries)),
    [analyzedEntries, providerAccuracy],
  );
              ...
              <ScrollArea.Autosize mah={420}>
                <Box className={classes.gameList}>
                  {games.map((game, index) => {
                    ...
                    const accuracy = findAccuracyForUrl(analyzedAccuracy, game.url);
                    return (
                      <Box className={classes.gameCard} key={`${game.url}-${index}`}>
                        <Box className={classes.gameDetails}>
                          <Group gap={6} wrap="nowrap">
                            ...
                            <Badge
                              color={accuracy == null ? "gray" : "green"}
                              size="xs"
                              variant="light"
                            >
                              {accuracy == null

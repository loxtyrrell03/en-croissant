    ...
    for (const result of imported) {
        ...
        for (const database of state.databases) {
            ...
            if (previousGames.length === 0)
                previousGames = state.gamesByDatabase[database.id] ?? [];
            replacedIds.add(database.id);
            replacementIds.set(database.id, result.database.id);
            delete nextGames[database.id];
        }

        nextGames[result.database.id] = carryForwardWebCoachReviews(previousGames, result.games);
    }
...
function carryForwardWebCoachReviews(previousGames: WebGame[], nextGames: WebGame[]) {
    if (previousGames.length === 0) return nextGames;
    const previousByIndex = new Map(previousGames.map((game) => [game.index, game]));
    return nextGames.map((game) => {
        const previous = previousByIndex.get(game.index);
        return previous?.coachReview && webGamesMatchForCoachReview(previous, game)
            ? { ...game, coachReview: previous.coachReview }
            : game;
    });
}

function webGamesMatchForCoachReview(previous: WebGame, next: WebGame) {
    return (
        ...
        previous.pgn.trim() === next.pgn.trim() &&
        previous.moves.length === next.moves.length &&
        previous.moves.every((move, index) => {
            const nextMove = next.moves[index];
            return (
                Boolean(nextMove) &&
                move.san === nextMove.san &&
                move.uci === nextMove.uci &&
                normalizeReviewFen(move.fenBefore) === normalizeReviewFen(nextMove.fenBefore) &&
                normalizeReviewFen(move.fenAfter) === normalizeReviewFen(nextMove.fenAfter)
            );
        })
    );
}

function normalizeReviewFen(fen: string) {
...
export function resolveWebDatabaseSourceId(
    ...
    if (value.startsWith(HOSTED_SOURCE_REF_PREFIX)) {
        ...
        return (
            databases.find(
                (database) => normalizeHostedPath(database.hostedPath ?? "") === hostedPath,
            )?.id ?? null
        );
    }

...
import { loadSharedLichessCredential, saveSharedLichessCredential } from "../sharedLichessAuth";
...
describe("shared Lichess authentication", () => {
    ...
    it("stops waiting when the private host does not answer", async () => {
        ...
        await expect(loadSharedLichessCredential({ timeoutMs: 1 })).rejects.toMatchObject({
            name: "AbortError",
        });
    });
});

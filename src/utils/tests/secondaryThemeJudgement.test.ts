...
test("frozen engine and source inputs retain unrelated judgements across the full secondary sample", () => {
    ...
    const changed = new Set(["lichess:qQG5v", "lichess:sKDBG", "lichess:xxDaj", "lichess:SD5oo"]);
    const cases = receipt.cases.map((r: any) => {
        ...
        expect(scan.motifs[0]?.id).toBe(
            r.id === "lichess:SD5oo" ? "mateIn2" : r.scan.motifs[0]?.id,
        );
    ...
    if (process.env.TACTICAL_SECONDARY_FINAL_REPORT)
        writeFileSync(
            ...
            JSON.stringify(
                {
                    scope: "Exact engine/source-input replay. Four changed explanations reviewed separately; unchanged/empty outputs are not accuracy successes.",
                    cases,
                },
                null,
                2,

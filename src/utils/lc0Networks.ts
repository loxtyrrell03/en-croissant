...
export function readEngineSetting(
...
export function replaceEngineSetting(
  settings: { name: string; value: string | number | boolean | null }[],
  name: string,
  value: string | number | boolean,
) {
    const found = settings.some((setting) => setting.name === name);
    return found
        ? settings.map((setting) => (setting.name === name ? { ...setting, value } : setting))
        : [...settings, { name, value }];
}

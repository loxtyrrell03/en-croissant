...
async fn scan_lichess_fide_broadcasts(
    ...
    let specs = resolve_lichess_round_specs(
        client,
        request,
        round_paths,
        &corpus_coverage,
...
async fn resolve_lichess_round_specs(
    ...
    while let Some((round_path, _api_url, result)) = lookups.next().await {
        let Some(round_id) = round_path.rsplit('/').next().filter(|id| !id.is_empty()) else {
            continue;
        };
        let fallback_url = format!("https://lichess.org/api/broadcast/round/{round_id}.pgn");
        let fallback_cache_path = request
            .cache_dir
            ...
            Ok((_, value)) => {
                if let Some(chapter_ids) = lichess_player_chapter_ids(&value, identity) {
                    for chapter_id in chapter_ids {
                        let url =
                            format!("https://lichess.org/api/study/{round_id}/{chapter_id}.pgn");
                        let cache_path = request
                            .cache_dir
                            .join(cache_file_name("lichess-broadcast-game", &url));
                        specs.insert(
                            url.clone(),
                    ...
                    specs.insert(
                        fallback_url.clone(),
                        IndexedArchiveSpec::lichess(fallback_url, fallback_cache_path),
                    );
                }
            }
            Err(_error) => {
                // Metadata is only an optimization. The original whole-round
                // request remains authoritative, so report a coverage error
                // only if that fallback also fails during the direct scan.
                specs.insert(
                    fallback_url.clone(),
                    IndexedArchiveSpec::lichess(fallback_url, fallback_cache_path),
                );
...
fn canonicalize_san_word(word: &str) -> SanWord {
    let core = if let Some(core) = word.strip_suffix("++") {
        core
    ...
    if matches!(castle.as_str(), "O-O" | "O-O-O") {
        return SanWord::Move(castle);
    }
    if core == "--" {
        return SanWord::Move(core.to_string());
    }
    if let Some(drop) = canonical_san_drop(core) {
        return SanWord::Move(drop.to_string());
    }
...
"#;
    ...
    #[test]
    fn check_and_mate_suffixes_share_the_same_move_fingerprint() {
        let check = "[Result \"1-0\"]\n\n1. e4 e5 2. Nf3+ Nc6 1-0";
        let mate = check.replace("Nf3+", "Nf3#");
        assert_eq!(
            mainline_move_fingerprint(check).moves,
            mainline_move_fingerprint(&mate).moves
        );
    }
...
<td><a href="https://theweekinchess.com/zips/twic1652g.zip">PGN</a></td>
    ...
    #[test]
    fn collector_deduplicates_archives_that_only_disagree_on_check_vs_mate() {
        let check = "[Event \"Open\"]\n[Date \"2026.05.10\"]\n[Round \"1\"]\n[White \"Target\"]\n[Black \"Opponent\"]\n[Result \"1-0\"]\n\n1. e4 e5 2. Nf3+ Nc6 1-0\n";
        let mate = check.replace("Nf3+", "Nf3#");
        let mut collection = Collection::default();
        add_game(&mut collection, check.to_string(), "A", "White");
        add_game(&mut collection, mate, "B", "White");

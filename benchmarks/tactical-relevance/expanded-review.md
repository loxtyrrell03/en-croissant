...
| NGZzo | Double Threat at Nd7 (adapter 24): newly attacks Rb6 and threatens Nf6+, forking Kg8/Qh5. A root-only proof checks all 35 replies and must require both capture and fork routes against different material. Rb7 permits Nf6+; Kg7 permits Nxb6. The root board shows the real rook attack, with a conditional fork at ply 3 only when played. Fresh before/after searches verify the actual g6 error and a missed Nd7. This differs from the source's trapped-piece tag: the rook can escape, but cannot also remove the separate checking fork. |
...
| opGD7 | Forcing Attack at Qf1+ (adapter 25), including the three-ply source and root-only input. All Kc2, Kd2 and Re1 defences permit a verified material gain; the bounded proof guarantees at least an exchange, not the engine's full +614 cp score. Fresh searches show the stronger Re3 Qf2+ Kd1 Bxe3 continuation after Kd2 and mate after Kc2/Re1. A branch's later pin is not an unavoidable root pin. Actual-ply fork/pin/capture or checkmate events remain conditional; Qf2+ now describes an already-pinned rook. Missing bishop/checker-capture/immediately-available-capture controls abstain. Fresh mistake checks distinguish allowed Qf1+ after Re2, missed Qf1+ after a5, and Be7's simpler hanging-bishop headline. |
...
| MJZcU | Resolved locally in adapter 29: Rxe4 removes Ne4's legal control of d2. After fxe4, the connected c/d passed pawns overcome the rook. All 21 root replies, including checking defences, pass the bounded material proof; the shortest source line promotes c1=Q, while fresh Stockfish instead uses d1=Q. Depth-16 MultiPV3 prefers Rxe4 (+267 cp) over premature c2 (-172 cp), when Rd7+ and Rc7 retain a blockade. Promotion Combination is primary, promotion remains at its actual ply, and later queen captures are excluded. This does not establish every branch's full-game outcome. |
| 0RleZ | Core sound: Re7# is supported by Nd5, itself supported by e4; hook geometry is present. |
| Z5arb | Open root miss: Nf3+ geometrically attacks king and queen while uncovering Bf6 against Nc3. Fresh depth-16 evidence prefers Nf3+ (+265 cp) to Qc6 (-135 cp). After gxf3 Bxc3, Qxc3 Qxf3 combines king pressure with the d3 passer, while Qf4 permits d2. Neither branch establishes a simple net-material fork, so the result remains unresolved rather than weakening fork validation. |
...
| eOCp9 | Open quiet king attack. Fresh depth-16 MultiPV3 prefers Qf4 (+277 cp), with g5 Rxg7+ Kxg7 Qe5+ Kh7 Bd4; the root-restricted Bf8 Qe5 line instead involves Bd4/Rxd4. The queen manoeuvre and bishop battery are plausible attacking mechanisms, but neither the source line nor these different defensive continuations supplies the classifier's required bounded proof. It remains unresolved. |

...
## Adapter 44: quiet offers with concrete defensive resources

Two private examples now explain the quiet capture before a checking fork, including a bishop-for-knight pawn gain and a harder sequence requiring a checking countercapture recovery. Every acceptance/decline branch is checked; the material recapturer must be the fork's payoff, and an attacked forker may save itself while defending the offer. Quiet searches have an 8,192-move allowance; checking searches retain 4,096. The current board shows only the actual capture, not a nonexistent check. See [private-course-review.md](private-course-review.md) for ten fresh engine checks, thresholds, cache/budget controls and remaining limits. Verification: 504 selected tests, 89 public plus six private actual-controller worker cases, isolated HTTP execution and production/shared builds. This improves two explanations; it is not new holdout accuracy, native deployment or physical UI proof.
